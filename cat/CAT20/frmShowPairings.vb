...
Public Class frmShowPairings
    ...
    Private Sub butLoad_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butLoad.Click
        ...
        Label1.Text = "Load COMPLETE" : Label1.Refresh()
    End Sub
    Sub LoadTeamTable()
    ...
    Sub ShowThePairing()
        ...
        If chkHideNames.Checked = True Then Call HideNames()
        ...
        For x = 0 To DataGridView1.RowCount - 1
            For y = 0 To DataGridView1.Columns.Count - 1
                If InStr(DataGridView1.Columns(y).Name.ToUpper, "TEAMNAME") > 0 Then
                    DataGridView1.Rows(x).Cells(y).Value = "XXXX"
                End If
            Next y
        Next x
    End Sub
    ...
    Private Sub Grid1Clicked() Handles DataGridView1.MouseClick
        grpJudgeSettings.Visible = False : grpRoomOptions.Visible = False : grpLoadSettings.Visible = False
        ...
        ElseIf InStr(DataGridView1.Columns(DataGridView1.CurrentCell.ColumnIndex).Name.ToUpper, "ROOM") > 0 Then
            ...
            grpJudgeSettings.Visible = False : grpLoadSettings.Visible = False
    ...
    Sub ShowRatings()
        ...
        'check that ratings are in use
        Dim dummy, dummy2 As String
        Label1.Text = ""
        For x = 0 To DataGridView1.ColumnCount - 1
            dummy = "" : dummy2 = ""
            If Mid(DataGridView1.Columns(x).Name, 1, 9) = "JUDGENAME" Then
                dummy = DataGridView1.CurrentRow.Cells(x).Value.ToString & " "
                'retrieve and show them in the message box
                Dim JudgeID = DataGridView1.CurrentRow.Cells(x - 1).Value
                For y = 1 To TeamsPerRound
                    If dummy2 <> "" Then dummy2 &= " - "
                    dummy2 &= GetJudgeRating(ds, JudgeID, DataGridView1.CurrentRow.Cells("Team" & y).Value, drRd.Item("JudgePlaceScheme"))
                    If ds.Tables("JudgePref").Columns.Contains("OrdCat") And chkOrdCats.Checked = False Then
                        chkOrdCats.Checked = True
                        dummy2 &= " (" & GetJudgeRating(ds, JudgeID, DataGridView1.CurrentRow.Cells("Team" & y).Value, drRd.Item("JudgePlaceScheme")) & ")"
                        chkOrdCats.Checked = False
                    ElseIf ds.Tables("JudgePref").Columns.Contains("OrdCat") And chkOrdCats.Checked = True Then
                        chkOrdCats.Checked = False
                        dummy2 &= " (" & GetJudgeRating(ds, JudgeID, DataGridView1.CurrentRow.Cells("Team" & y).Value, drRd.Item("JudgePlaceScheme")) & ")"
                        chkOrdCats.Checked = True
                    End If
                Next y
                Label1.Text &= dummy & " " & dummy2 & Chr(10)
            End If
        Next x
    End Sub
    ...
    Sub LoadJudges()
        ...
        If drRd.Item("Flighting") < 1 Or drRd.Item("Flighting") Is Nothing Or drRd.Item("Flighting") Is System.DBNull.Value Then
            MsgBox("The flighting settings for this round and incomplete; please go to the round setup screen, enter a flighting value of at least 1, and return.")
            Exit Sub
        End If
        z = drRd.Item("Flighting")
        If chkSingleChangeFlights.Checked = True Then z = 1
        For x = 1 To (TeamsPerRound * z)
        ...
        For x = 0 To ds.Tables("Judge").Rows.Count - 1
            ...
            If ds.Tables("Judge").Rows(x).Item("StopScheduling") = False Then
                ...
                For y = 1 To TeamsPerRound
                    ...
                    End If
                Next y
                If DataGridView1.CurrentRow.Cells("Room").Value Is System.DBNull.Value Then DataGridView1.CurrentRow.Cells("Room").Value = -1
                ...
                If drRd.Item("Flighting") > 1 And chkSingleChangeFlights.Checked = False Then 'add additional ratings if flighting
                ...
                End If
                dr2.Item("Pref") = -999
                y = drRd.Item("Flighting")
                If chkSingleChangeFlights.Checked = True Then y = 1
                For z = 1 To (TeamsPerRound * y)
                    If dr2.Item("Rating" & z) Is System.DBNull.Value Then dr2.Item("Rating" & z) = 0
                    If dr2.Item("Rating" & z) > dr2.Item("Pref") Then dr2.Item("Pref") = dr2.Item("Rating" & z)
                Next z
                dr2.Item("Mutuality") = 0
                For z = 1 To (TeamsPerRound * y) Step 2
                    If Math.Abs(dr2.Item("Rating" & z) - dr2.Item("Rating" & z + 1)) > dr2.Item("Mutuality") Then dr2.Item("Mutuality") = Math.Abs(dr2.Item("Rating" & z) - dr2.Item("Rating" & z + 1))
                Next z
                ...
                'now check judgedbefore column for flighting
                If chkSingleChangeFlights.Checked = False Then
                    For z = 0 To fdOtherRooms.Length - 1
                        If dr2.Item("JudgedBefore") = False Then
                            dr2.Item("JudgedBefore") = JudgedBefore(ds, ds.Tables("Judge").Rows(x).Item("ID"), fdOtherRooms(z).Item("Panel"))
                        End If
                    Next z
                End If
                dt.Rows.Add(dr2)
        ...
        DataGridView2.Columns.Clear()
        If chkHearOnce.Checked = True Then
            dt.DefaultView.Sort = "Assigned, JudgedBefore, FitsCriteria desc, Left desc, Pref"
        Else
    ...
    Private Sub butAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butAdd.Click
        ...
        If EditMode = "Judge" Then
            ...
            If q = vbNo Then Exit Sub
            If DataGridView2.CurrentRow.Cells("JudgedBefore").Value = True And chkHearOnce.Checked = True Then q = MsgBox("Judge has heard one or both teams before. " & JudgedBeforeDetails() & "Place anyway?", MsgBoxStyle.YesNo)
            If q = vbNo Then Exit Sub
            struck = False
    ...
    End Sub
    Function JudgedBeforeDetails() As String
        Dim dummy As String = ""
        Dim judge As Integer = DataGridView2.CurrentRow.Cells("ID").Value
        Dim drRoom As Integer = DataGridView1.CurrentRow.Cells("Room").Value
        Dim drCurrentRd As Integer = cboRound.SelectedValue
        Dim fdpanel As DataRow()
        If drRoom <> Nothing Then
            fdpanel = ds.Tables("panel").Select("Room=" & drRoom & " and round=" & drCurrentRd)
        Else
            fdpanel = ds.Tables("panel").Select("Room=" & DataGridView1.CurrentRow.Cells("Panel").Value)
        End If
        For z = 0 To fdpanel.Length - 1
            Dim fdBallot As DataRow() : Dim drEvent, drRd, drPanel, drentry As DataRow
            fdBallot = ds.Tables("Ballot").Select("panel=" & fdpanel(z).Item("ID"))
            Dim fdPrevious As DataRow()
            For x = 0 To fdBallot.Length - 1
                fdPrevious = ds.Tables("Ballot").Select("Judge=" & judge & " and entry=" & fdBallot(x).Item("Entry") & " and panel<>" & fdpanel(z).Item("ID"))
                For y = 0 To fdPrevious.Length - 1
                    drentry = ds.Tables("Entry").Rows.Find(fdPrevious(y).Item("entry"))
                    drPanel = ds.Tables("Panel").Rows.Find(fdPrevious(y).Item("Panel"))
                    drRd = ds.Tables("Round").Rows.Find(drPanel.Item("Round"))
                    drEvent = ds.Tables("Event").Rows.Find(drRd.Item("Event"))
                    dummy &= Chr(10) & drentry.Item("FullName") & " " & drRd.Item("Label") & " " & GetSideString(ds, fdPrevious(y).Item("Side"), drEvent.Item("ID"))
                Next y
            Next x
        Next z
        If dummy <> "" Then dummy &= Chr(10)
        JudgedBeforeDetails = dummy
    End Function
    Sub ClearTheGrids()
        For x = 0 To DataGridView1.RowCount - 1
            DataGridView1.Rows(x).Selected = False
    ...
    Sub ColorCodeAssignedJudges()
        Dim drRound As DataRow
        drRound = ds.Tables("Round").Rows.Find(cboRound.SelectedValue)
        Dim njudges As Integer = drRound.Item("JudgesPerPanel")
        Dim mut, rat1, rat2, PrefMax, MutMax As Integer
        PrefMax = DataGridView3.Rows(0).Cells(1).Value
        MutMax = DataGridView3.Rows(1).Cells(1).Value
        For z = 0 To DataGridView1.RowCount - 1
            For y = 1 To njudges
                If DataGridView1.CurrentRow.Cells("Judge" & y.ToString).Value > 1 Then
                    rat1 = GetJudgeRating(ds, DataGridView1.Rows(z).Cells("Judge" & y).Value, DataGridView1.Rows(z).Cells("Team1").Value, drRound.Item("JudgePlaceScheme"))
                    rat2 = GetJudgeRating(ds, DataGridView1.Rows(z).Cells("Judge" & y).Value, DataGridView1.Rows(z).Cells("Team2").Value, drRound.Item("JudgePlaceScheme"))
                    If rat1 > Int(PrefMax * 0.7) Or rat2 > Int(PrefMax * 0.7) Or mut > Int(MutMax * 0.5) Then DataGridView1.Item("JudgeName" & y, z).Style.BackColor = Color.LightGreen
                    If rat1 > Int(PrefMax * 0.7) And rat2 > Int(PrefMax * 0.7) And mut < Int(MutMax * 0.5) Then DataGridView1.Item("JudgeName" & y, z).Style.BackColor = Color.LightYellow
                    If rat1 > Int(PrefMax * 0.7) And rat2 > Int(PrefMax * 0.7) And mut > Int(MutMax * 0.5) Then DataGridView1.Item("JudgeName" & y, z).Style.BackColor = Color.Orange
                    If mut > MutMax Or rat1 > PrefMax Or rat2 > PrefMax Then DataGridView1.Item("JudgeName" & y, z).Style.BackColor = Color.Red
                End If
            Next y
        Next z
    End Sub
    ...
    Private Sub butPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butPrint.Click
        ...
        End Try
        If drTimeSlot.Item("Start").ToString = "" Then drTimeSlot.Item("Start") = Now
    ...
    Private Sub butJudgePlaceHelp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butJudgePlaceHelp.Click
        ...
        strInfo &= "The JUDGE RP button will launch the judge report, which is also available on the judge entry screen.  It will show the number of rounds needed and available." & Chr(10) & Chr(10)
        strInfo &= "The ORD RPT button will launch ordinal placement report, which is also available on the STA auto judge placement screen." & Chr(10) & Chr(10)
    ...
    Private Sub butLoadSettingHelp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butLoadSettingHelp.Click
        ...
        strInfo &= "JUDGE SCHOOL will display school affiliation of a judge in the judges box on the right-hand side." & Chr(10) & Chr(10)
        strInfo &= "ORDINAL CATS should be used if ordinal percentiles have been converted to categories on the utilities screen; checking the box will display the category equivalents rather than raw ordinal percentiles.  This is primarily used at the NDT." & Chr(10) & Chr(10)
        ...
        strInfo &= "HIDE NAMES will suppress team names, replacing them with an XXXX, so judge placement decisions can be made without knowing which teams they affect." & Chr(10) & Chr(10)
        strInfo &= "NO JUDGE FIRST NAMES will suppress judge first names, although they will still appear if a judge has a duplicate last name. " & Chr(10) & Chr(10)
    ...
    Private Sub butPageHelp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butPageHelp.Click
        ...
        strInfo &= "LOADING and format: To change the load settings click the EDIT LOAD SETTINGS and adjust them in the lower-right-hand box. Then select a round in the top left and click the 'Load Round' button." & Chr(10) & Chr(10)
        ...
        strInfo &= "Clicking the ORD->RAT button will turn all the ordinal percentiles into the entered ratings.  By default, all judge placements are based on ordinal ratings, so this essentially makes percentiles the same as ratings and thus uses the ratings. " & Chr(10) & Chr(10)
        strInfo &= "Clicking the ASSIGN BYE or SIDE CHANGE button will perform the appropriate action on the pairings.  " & Chr(10) & Chr(10)
        strInfo &= "Clicking the BEST ELIM JUDGES button will convert the pairings grid into a list of the best judges; this should only be used for elims, is only necessary if mutual preference judging is in use, and should probably not be used before quarters.  It's primary use is at the NDT.  " & Chr(10) & Chr(10)
    ...
    Private Sub butOrdsToRatings_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butOrdsToRatings.Click
        Dim q = MsgBox("This will convert all your ordinals to their raw ratings.  ONLY do this if you are using a categorical (tiered, e.g., 1-6 or 1-9 rating scheme).  Continue?", MsgBoxStyle.YesNo)
        If q <> vbYes Then Exit Sub
        For x = 0 To ds.Tables("JudgePref").Rows.Count - 1
            ds.Tables("JudgePref").Rows(x).Item("OrdPct") = ds.Tables("JudgePref").Rows(x).Item("Rating")
        Next x
        If DataGridView1.Rows.Count > 0 Then Call ShowThePairing()
        MsgBox("Done")
    End Sub

    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        grpJudgeSettings.Visible = False
        grpRoomOptions.Visible = False
        grpLoadSettings.Visible = True : grpLoadSettings.Location = New Point(1013, 480)
        lblGridKey.Text = "Change options and click the load button."
    End Sub
    ...
    Private Sub butChangeFont_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butChangeFont.Click
        ...
        If x < 6 Then x = 6
        If x > 14 Then x = 14
        Dim strWhich As String = InputBox("L for left grid, R for right grid, or B for both")
        If strWhich <> "R" Then
            DataGridView1.AutoResizeRows()
            DataGridView1.Font = New Font("Franklin Gothic Medium", x)
            Dim height As Single = Font.Height
            DataGridView1.RowTemplate.Height = height + 1
            MsgBox("Sort the grid or reload the round to resize the rows")
        End If
        If strWhich <> "L" Then
            DataGridView2.Font = New Font("Franklin Gothic Medium", x)
            DataGridView2.AutoResizeRows()
        End If
    End Sub

    Private Sub butBestElimFits_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butBestElimFits.Click
        If DataGridView1.RowCount < 1 Then
            MsgBox("Please load a round first")
            Exit Sub
        End If
        'exit if not an elim round
        Dim drRd As DataRow
        drRd = ds.Tables("Round").Rows.Find(cboRound.SelectedValue)
        If drRd.Item("Rd_Name") < 9 Then MsgBox("This function is only for elims.") : Exit Sub
        'load teams and elim seeds
        Dim fdSeeds() As DataRow
        fdSeeds = ds.Tables("ElimSeed").Select("Round=" & drRd.Item("ID"), "Seed ASC")
        'build a table with all possible future debates as columns and all judges as rows
        Dim Team1 As DataRow : Dim team2 As DataRow : Dim team3 As DataRow : Dim team4 As DataRow
        'team1=higher seed, team2=possible opponent #1, team3=possible opponent #2, team4=lower seed
        team2 = Nothing : team3 = Nothing : team4 = Nothing
        Dim i, n, col, seed1, seed2 As Integer
        Dim dt As New DataTable
        Dim teams((DataGridView1.RowCount * 4), 2)
        dt.Columns.Add("Judge", System.Type.GetType("System.String"))
        For x = 0 To ((fdSeeds.Length - 1) / 2) / 2
            'add column for first possible opponent
            Team1 = ds.Tables("Entry").Rows.Find(fdSeeds(x).Item("Entry"))
            n = fdSeeds.Length
            seed1 = ((n / 2) + 1) - fdSeeds(x).Item("Seed")
            For y = 0 To fdSeeds.Length - 1
                If fdSeeds(y).Item("Seed") = seed1 Then team2 = ds.Tables("Entry").Rows.Find(fdSeeds(y).Item("Entry")) : Exit For
            Next y
            dt.Columns.Add(x + 1 & "-" & seed1 & Chr(10) & Team1.Item("Code") & "-" & team2.Item("Code"), System.Type.GetType("System.String"))
            col = col + 1 : teams(col, 1) = Team1.Item("ID") : teams(col, 2) = team2.Item("ID")
            'add column for second possible opponent
            seed2 = (n + 1) - seed1
            For y = 0 To fdSeeds.Length - 1
                If fdSeeds(y).Item("Seed") = seed2 Then team3 = ds.Tables("Entry").Rows.Find(fdSeeds(y).Item("Entry")) : Exit For
            Next y
            dt.Columns.Add(x + 1 & "-" & seed2 & Chr(10) & Team1.Item("Code") & "-" & team3.Item("Code"), System.Type.GetType("System.String"))
            col = col + 1 : teams(col, 1) = Team1.Item("ID") : teams(col, 2) = team3.Item("ID")
            'add column if higher seed loses
            i = (n + 1) - fdSeeds(x).Item("Seed")
            For y = 0 To fdSeeds.Length - 1
                If fdSeeds(y).Item("Seed") = i Then team4 = ds.Tables("Entry").Rows.Find(fdSeeds(y).Item("Entry")) : Exit For
            Next y
            dt.Columns.Add(i & "-" & seed1 & Chr(10) & team4.Item("Code") & "-" & team2.Item("Code"), System.Type.GetType("System.String"))
            col = col + 1 : teams(col, 1) = team4.Item("ID") : teams(col, 2) = team2.Item("ID")
            dt.Columns.Add(i & "-" & seed2 & Chr(10) & team4.Item("Code") & "-" & team3.Item("Code"), System.Type.GetType("System.String"))
            col = col + 1 : teams(col, 1) = team4.Item("ID") : teams(col, 2) = team3.Item("ID")
        Next x
        dt.Columns.Add("PrefAvg", System.Type.GetType("System.Single"))
        dt.Columns.Add("TotalFits", System.Type.GetType("System.Int16"))
        'scroll each judge and show fit, plus average for teams remaining
        Dim dr As DataRow : Dim rat1, rat2 As Single
        Dim fits As Integer
        Dim PrefMax As Single = DataGridView3.Rows(0).Cells(1).Value
        Dim MutMax As Single = DataGridView3.Rows(1).Cells(1).Value
        Dim totpref As Single : Dim prefct As Integer : Dim nFits(col) As Integer
        For x = 0 To ds.Tables("Judge").Rows.Count - 1
            fits = 0
            dr = dt.NewRow
            dr.Item("Judge") = ds.Tables("Judge").Rows(x).Item("Last") & ", " & ds.Tables("Judge").Rows(x).Item("First")
            For y = 1 To col
                rat1 = CInt(GetJudgeRating(ds, ds.Tables("Judge").Rows(x).Item("ID"), teams(y, 1), drRd.Item("JudgePlaceScheme")))
                rat2 = CInt(GetJudgeRating(ds, ds.Tables("Judge").Rows(x).Item("ID"), teams(y, 2), drRd.Item("JudgePlaceScheme")))
                If rat1 = 999 Then dr.Item(y) = "X" Else dr.Item(y) = rat1
                If rat2 = 999 Then dr.Item(y) &= "-X" Else dr.Item(y) &= "-" & rat2
                If rat1 < PrefMax And rat2 < PrefMax And Math.Abs(rat1 - rat2) <= MutMax Then fits += 1 : dr.Item(y) &= "*" : nFits(y) += 1
            Next y
            dr.Item("TotalFits") = fits
            totpref = 0 : prefct = 0
            For y = 0 To fdSeeds.Length - 1
                rat1 = GetJudgeRating(ds, ds.Tables("Judge").Rows(x).Item("ID"), fdSeeds(y).Item("Entry"), drRd.Item("JudgePlaceScheme"))
                If rat1 < 999 Then
                    totpref += rat1 : prefct += 1
                End If
            Next y
            dr.Item("PrefAvg") = FormatNumber(totpref / prefct, 1)
            dt.Rows.Add(dr)
        Next x
        'add summary row
        dr = dt.NewRow
        For y = 1 To col
            dr.Item(y) = nFits(y)
        Next y
        dr.Item(0) = "Total fits"
        dt.Rows.Add(dr)
        'sort and bind
        dt.DefaultView.Sort = "TotalFits desc, PrefAvg asc"
    ...
    Private Sub butStrikeCards_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butStrikeCards.Click
        ...
        For x = 0 To DataGridView1.RowCount - 1
            For y = 0 To DataGridView1.Columns.Count - 1
                If Mid(DataGridView1.Columns(y).Name.ToString, 1, 8).ToUpper = "TEAMNAME" Then
                    dr = dt.NewRow
                    dr.Item("Content") = DataGridView1.Rows(x).Cells(y).Value
                    dt.Rows.Add(dr)
                    For z = 0 To DataGridView1.ColumnCount - 1
                        If Mid(DataGridView1.Columns(z).Name.ToString, 1, 9).ToUpper = "JUDGENAME" Then
                            dr = dt.NewRow
                            dr.Item("Content") = DataGridView1.Rows(x).Cells(z).Value
                            dt.Rows.Add(dr)
                        End If
                    Next z
                    dr = dt.NewRow
                    dr.Item("Content") = "<br clear=all style=page-break-before:always />"
                    dt.Rows.Add(dr)
                End If
            Next y
        Next x
        DataGridView1.Columns.Clear()
        DataGridView1.DataSource = dt
        MsgBox("Done -- hit the print button")
    End Sub
End Class
