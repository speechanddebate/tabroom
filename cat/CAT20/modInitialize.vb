...
Module modInitialize
    ...
    Sub SetupCompletionStatus(ByVal ds As DataSet)
        ...
        For x = 0 To ds.Tables("Round").Rows.Count - 1
            fdTB = ds.Tables("Tiebreak_Set").Rows.Find(ds.Tables("Round").Rows(x).Item("TB_Set"))
            If fdTB Is Nothing Then
                frmMainMenu.lblSetupStatus.Text = "One or more rounds is not linked to a tiebreaker set. First visit the SET UP TIEBREAKERS screen and make sure team tiebreakers are set.  Then click on the SET UP ROUND SCHEDULE button to link tiebreaker sets to rounds.  More information can be found by clicking the help buttons on setup pages 3 and 4." : Exit Sub
            End If
        Next x
    ...
    Sub AddSTAResults()
        ...
        xmlFile.Close()
        'STEP TWO: Load new files without validation
        Dim dsSTA As New DataSet
        xmlFile = XmlReader.Create(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\CAT\XML DATA.xml", New XmlReaderSettings())
        dsSTA.ReadXml(xmlFile, XmlReadMode.InferSchema)
        xmlFile.Close()
        'STEP THREE: Validate recipients in new ballot_score file
        Dim dr As DataRow()
        For x = 0 To dsSTA.Tables("Ballot_Score").Rows.Count - 1
            If dsSTA.Tables("Ballot_Score").Rows(x).Item("Score_ID") = 1 And dsSTA.Tables("Ballot_Score").Rows(x).Item("Recipient") <> -99 Then
                dr = ds.Tables("Entry").Select("ID=" & dsSTA.Tables("Ballot_Score").Rows(x).Item("Recipient"))
                If dr.Length = 0 Then
                    MsgBox("No entry for team " & dsSTA.Tables("Ballot_Score").Rows(x).Item("Recipient"))
                End If
            ElseIf dsSTA.Tables("Ballot_Score").Rows(x).Item("Score_ID") = 2 And dsSTA.Tables("Ballot_Score").Rows(x).Item("Recipient") <> -99 Then
                dr = ds.Tables("Entry_Student").Select("ID=" & dsSTA.Tables("Ballot_Score").Rows(x).Item("Recipient"))
                If dr.Length = 0 Then
                    MsgBox("No entry for speaker " & dsSTA.Tables("Ballot_Score").Rows(x).Item("Recipient"))
                End If
            End If
        Next
        'Validate judges
        For x = 0 To dsSTA.Tables("Ballot").Rows.Count - 1
            If dsSTA.Tables("Ballot").Rows(x).Item("Judge") = "" Then dsSTA.Tables("Ballot").Rows(x).Item("Judge") = -99
            If dsSTA.Tables("Ballot").Rows(x).Item("Judge") <> -99 Then
                dr = ds.Tables("Judge").Select("ID=" & dsSTA.Tables("Ballot").Rows(x).Item("Judge"))
                If dr.Length = 0 Then
                    MsgBox("No entry for judge " & dsSTA.Tables("Ballot").Rows(x).Item("Judge"))
                End If
            End If
        Next
        'STEP FOUR: Dump the STA files in
        Dim drNew As DataRow
        'Timeslots
        ds.Tables("Timeslot").Clear()
        For x = 1 To 16
            drNew = ds.Tables("TimeSlot").NewRow
            drNew.Item("TimeSlotName") = "TimeSlot # " & x
            drNew.Item("ID") = x
            ds.Tables("TimeSlot").Rows.Add(drNew)
        Next x
        'Event
        ds.Tables("Event").Clear()
        ds.Tables("Event_Setting").Clear()
        FieldCheck(dsSTA, "Event", "EventName", "System.String")
        For x = 0 To dsSTA.Tables("Event").Rows.Count - 1
            drNew = ds.Tables("Event").NewRow
            drNew.Item("ID") = dsSTA.Tables("Event").Rows(x).Item("ID")
            If dsSTA.Tables("Event").Rows(x).Item("EventName") Is System.DBNull.Value Then
                If dsSTA.Tables("Event").Rows(x).Item("ID") = 1 Then dsSTA.Tables("Event").Rows(x).Item("EventName") = "Open"
                If dsSTA.Tables("Event").Rows(x).Item("ID") = 2 Then dsSTA.Tables("Event").Rows(x).Item("EventName") = "JV"
            End If
            drNew.Item("EventName") = dsSTA.Tables("Event").Rows(x).Item("EventName")
            drNew.Item("ABBR") = drNew.Item("EventName")
            drNew.Item("Type") = "Policy"
            ds.Tables("Event").Rows.Add(drNew)
        Next x
        'Round
        ds.Tables("Round").Clear()
        FieldCheck(dsSTA, "Round", "Event", "System.Int64")
        For x = 0 To dsSTA.Tables("Round").Rows.Count - 1
            drNew = ds.Tables("Round").NewRow
            drNew.Item("ID") = dsSTA.Tables("Round").Rows(x).Item("ID")
            If dsSTA.Tables("Round").Rows(x).Item("Event") Is System.DBNull.Value Then
                If dsSTA.Tables("Round").Rows(x).Item("ID") < 200 Then dsSTA.Tables("Round").Rows(x).Item("Event") = 1
                If dsSTA.Tables("Round").Rows(x).Item("ID") > 200 Then dsSTA.Tables("Round").Rows(x).Item("Event") = 2
            End If
            drNew.Item("Event") = dsSTA.Tables("Round").Rows(x).Item("Event")
            drNew.Item("Label") = dsSTA.Tables("Round").Rows(x).Item("ID")
            drNew.Item("Timeslot") = dsSTA.Tables("Round").Rows(x).Item("Timeslot")
            drNew.Item("Rd_Name") = dsSTA.Tables("Round").Rows(x).Item("Rd_Name")
            drNew.Item("Flighting") = 1
            If drNew.Item("Rd_Name") <= 9 Then drNew.Item("JudgesPerPanel") = 1 Else drNew.Item("JudgesPerPanel") = 3
            drNew.Item("JudgePlaceScheme") = "TeamRating"
            drNew.Item("PairingScheme") = "Random"
            ds.Tables("Round").Rows.Add(drNew)
        Next x
        'Panel
        If ds.Tables.IndexOf("Panel") = -1 Then
            ds.Tables.Add("PANEL")
            FieldCheck(ds, "PANEL", "ID", "System.Int64")
            FieldCheck(ds, "PANEL", "ROUND", "System.Int64")
            FieldCheck(ds, "PANEL", "ROOM", "System.String")
            FieldCheck(ds, "PANEL", "FLIGHT", "System.Int64")
        End If
        ds.Tables("Panel").Clear()
        For x = 0 To dsSTA.Tables("Panel").Rows.Count - 1
            drNew = ds.Tables("Panel").NewRow
            drNew.Item("ID") = dsSTA.Tables("Panel").Rows(x).Item("ID")
            drNew.Item("Round") = dsSTA.Tables("Panel").Rows(x).Item("ROund")
            drNew.Item("Room") = -99
            drNew.Item("Flight") = 1
            ds.Tables("Panel").Rows.Add(drNew)
        Next x
        'validate panel IDs as unique
        For x = 0 To dsSTA.Tables("Panel").Rows.Count - 1
            dr = ds.Tables("Panel").Select("ID=" & dsSTA.Tables("Panel").Rows(x).Item("ID"))
            If dr.Length > 1 Then
                For y = dr.Length - 1 To 1 Step -1
                    dr(y).Delete()
                Next
            End If
        Next
        'Validate a round for each panel
        For x = 0 To dsSTA.Tables("Panel").Rows.Count - 1
            dr = ds.Tables("Round").Select("ID='" & dsSTA.Tables("Panel").Rows(x).Item("Round") & "'")
            If dr.Length = 0 Then
                MsgBox("No round for panel " & dsSTA.Tables("Panel").Rows(x).Item("ID"))
            End If
        Next
        'Ballot
        If ds.Tables.IndexOf("BALLOT") = -1 Then
            ds.Tables.Add("BALLOT")
            FieldCheck(ds, "Ballot", "ID", "System.Int64")
            FieldCheck(ds, "Ballot", "JUDGE", "System.Int64")
            FieldCheck(ds, "Ballot", "PANEL", "System.Int64")
            FieldCheck(ds, "Ballot", "ENTRY", "System.Int64")
            FieldCheck(ds, "Ballot", "SIDE", "System.Int64")
        End If
        ds.Tables("Ballot").Clear()
        For x = 0 To dsSTA.Tables("Ballot").Rows.Count - 1
            drNew = ds.Tables("Ballot").NewRow
            drNew.Item("ID") = dsSTA.Tables("Ballot").Rows(x).Item("ID")
            drNew.Item("Judge") = dsSTA.Tables("Ballot").Rows(x).Item("Judge")
            drNew.Item("Panel") = dsSTA.Tables("Ballot").Rows(x).Item("Panel")
            drNew.Item("Entry") = dsSTA.Tables("Ballot").Rows(x).Item("Entry")
            If dsSTA.Tables("Ballot").Rows(x).Item("SIde") = "A" Then drNew.Item("Side") = 1
            If dsSTA.Tables("Ballot").Rows(x).Item("SIde") = "N" Then drNew.Item("Side") = 2
            ds.Tables("Ballot").Rows.Add(drNew)
        Next x
        'validate panels for ballot
        For x = 0 To dsSTA.Tables("Ballot").Rows.Count - 1
            dr = ds.Tables("Panel").Select("ID=" & dsSTA.Tables("Ballot").Rows(x).Item("Panel"))
            If dr.Length = 0 Then
                MsgBox("No panel for ballot " & dsSTA.Tables("Ballot").Rows(x).Item("ID"))
            End If
        Next
        'Ballot_score
        If ds.Tables.IndexOf("BALLOT_SCORE") = -1 Then
            ds.Tables.Add("BALLOT_SCORE")
            FieldCheck(ds, "BALLOT_SCORE", "ID", "System.Int64")
            FieldCheck(ds, "BALLOT_SCORE", "BALLOT", "System.Int64")
            FieldCheck(ds, "BALLOT_SCORE", "RECIPIENT", "System.Int64")
            FieldCheck(ds, "BALLOT_SCORE", "SCORE_ID", "System.Int64")
            FieldCheck(ds, "BALLOT_SCORE", "SCORE", "System.Single")
        End If
        ds.Tables("BALLOT_SCORE").Clear()
        For x = 0 To dsSTA.Tables("Ballot_score").Rows.Count - 1
            drNew = ds.Tables("Ballot_Score").NewRow
            drNew.Item("ID") = dsSTA.Tables("Ballot_Score").Rows(x).Item("ID")
            drNew.Item("BALLOT") = dsSTA.Tables("Ballot_Score").Rows(x).Item("BALLOT")
            drNew.Item("RECIPIENT") = dsSTA.Tables("Ballot_Score").Rows(x).Item("RECIPIENT")
            drNew.Item("SCORE_ID") = dsSTA.Tables("Ballot_Score").Rows(x).Item("SCORE_ID")
            drNew.Item("SCORE") = dsSTA.Tables("Ballot_Score").Rows(x).Item("SCORE")
            ds.Tables("Ballot_Score").Rows.Add(drNew)
        Next x
        'Save and close
        Call SaveFile(ds)
        MsgBox("Done")
