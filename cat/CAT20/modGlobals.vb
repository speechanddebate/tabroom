...
Module modGlobals
    ...
    Public Sub LoadFile(ByRef ds As DataSet, ByVal strDataFileName As String)
        ...
        If strDataFileName = "TourneyData" Then
            ...
            'round to panel
            'Call RefCheck(ds, "ROUND", "PANEL", "ROUND")
                ...
                'EVENT to ENTRY
                Call RefCheck(ds, "Event", "Entry", "Event")
                If ds.Tables.IndexOf("EVENT") > -1 And ds.Tables.IndexOf("EVENT_SETTING") > -1 Then
                ...
                'Judge to JudgePref
                'Call RefCheck(ds, "Judge", "JudgePref", "Judge")
                Call PrefFixer(ds)
                If ds.Tables.IndexOf("TIEBREAK_SET") > -1 And ds.Tables.IndexOf("TIEBREAK") > -1 Then
        ...
        End If
    End Sub
    Sub ShowLoadError(ByVal DS)
        Dim rowsinerror() As DataRow
        For x = 0 To DS.Tables.Count - 1
            rowsinerror = DS.Tables(x).GetErrors()
            For i = 0 To rowsinerror.GetUpperBound(0)
                For Each column In DS.Tables(x).Columns
                    MsgBox("Error in " & DS.Tables(x).TableName & ":" & rowsinerror(i).RowError.ToString)
                Next
            Next i
        Next x
        MsgBox("Showing of errors complete")
    End Sub
    Private Sub PrintAllErrs(ByVal dataSet As DataSet)
            ...
            If table.HasErrors Then
                ...
                For i = 0 To rowsInError.GetUpperBound(0)
                    For Each column In table.Columns
                        MsgBox(column.ColumnName, rowsInError(i).GetColumnError(column))
                    Next
        ...
        Next
    End Sub
    Sub PrefFixer(ByVal ds As DataSet)
        Dim dr As DataRow
        For x = ds.Tables("JudgePref").Rows.Count - 1 To 0 Step -1
            dr = ds.Tables("Judge").Rows.Find(ds.Tables("JudgePref").Rows(x).Item("Judge"))
            If dr Is Nothing Then
                ds.Tables("JudgePref").Rows(x).Delete()
            End If
        Next x
    End Sub
    Sub RefCheck(ByVal ds As DataSet, ByVal strParent As String, ByVal strChild As String, ByVal strField As String)
        'parent table, child table, and child field (parent field is always ID)
        Dim dr As DataRow
        For x = 0 To ds.Tables(strChild).Rows.Count - 1
            dr = ds.Tables(strParent).Rows.Find(ds.Tables(strChild).Rows(x).Item(strField))
            If dr Is Nothing Then
                MsgBox("No match in " & strParent & " for " & strChild & " value of " & ds.Tables(strChild).Rows(x).Item(strField))
            End If
        Next x
    End Sub
    ...
    Public Function FullNameMaker(ByVal DS As DataSet, ByVal strTeam As Integer, ByVal strType As String, ByVal LnLength As Integer) As String
        ...
        Else
            If StudentRow.Count > 1 Then FullNameMaker &= " " & Mid(StudentRow(0).Item("Last"), 1, LnLength)
            If StudentRow.Count = 1 Then FullNameMaker &= " " & Mid(StudentRow(0).Item("First"), 1, LnLength) & Mid(StudentRow(0).Item("Last"), 1, LnLength)
        ...
        For x = 0 To fdPanel.Length - 1
            ...
            For y = 0 To fdBallot.Length - 1
                ...
                If y = 0 Then
                    ...
                    Else
                        dr.Item("JudgeName" & Judges) = GetName(DS.Tables("Judge"), fdBallot(y).Item("Judge"), "LAST")
                        If frmShowPairings.chkSuppressFirstNames.Checked = False Or DupeLastName(DS, fdBallot(y).Item("Judge")) = True Then
                            dr.Item("JudgeName" & Judges) &= ", " & GetName(DS.Tables("Judge"), fdBallot(y).Item("Judge"), "FIRST")
                        End If
                    End If
    ...
    End Function
    Function DupeLastName(ByVal ds As DataSet, ByVal JudgeID As Integer) As Boolean
        DupeLastName = False
        If JudgeID <= 0 Then Exit Function
        Dim dr As DataRow
        dr = ds.Tables("Judge").Rows.Find(JudgeID)
        Dim fdJudges() As DataRow
        Dim dummy As String = dr.Item("Last")
        dummy = dummy.Replace("'", "")
        fdJudges = ds.Tables("Judge").Select("Last='" & dummy & "'")
        If fdJudges.Length > 1 Then DupeLastName = True
    End Function
    ...
    Function GetLastCompleteRound(ByVal ds As DataSet, ByVal round As Integer) As Integer
        ...
        Dim fdRd As DataRow() : fdRd = ds.Tables("Round").Select("Event=" & drRd.Item("Event") & " and Rd_Name <=" & drRd.Item("Rd_Name"), "Rd_Name ASC")
    ...
    Function GetJudgeRating(ByVal ds As DataSet, ByVal Judge As Integer, ByVal Team As Integer, ByVal strRatingType As String) As Single
        ...
        'not using tab ratings, so return ordinal percentile or OrdCat, depending on what is specified
        ...
        If Not fdRow(0).Item("OrdPct") Is System.DBNull.Value Then
            ...
            If frmShowPairings.chkOrdCats.Checked = True And ds.Tables("JudgePref").Columns.Contains("OrdCat") = True And Not fdRow(0).Item("OrdCat") Is System.DBNull.Value Then GetJudgeRating = fdRow(0).Item("OrdCat")
    ...
    Sub MakeBallotFile(ByVal Ds As DataSet, ByVal Round As Integer, ByVal strStartTime As String, ByVal strSortBy As String, ByVal ElimInstr As Boolean)
        ...
        dtJudges.Columns.Add("Room", System.Type.GetType("System.Int64"))
        dtJudges.Columns.Add("Flight", System.Type.GetType("System.Int64"))
        ...
        For x = 0 To fdPanel.Length - 1
            ...
            Next y
        Next x
        dtJudges.DefaultView.Sort = "JudgeName ASC, Flight ASC"
        If strSortBy = "ROOM" Then dtJudges.DefaultView.Sort = "Room ASC, flight ASC"
        ...
        For x = 0 To dtJudges.DefaultView.Count - 1
            If dtJudges.DefaultView(x).Item("JudgeName") <> "NA" Then
                ...
                If drRound.Item("Flighting") > 1 Then
                    If dtJudges.DefaultView(x).Item("Flight") = 1 Then st.WriteLine("<b>Room:</b>" & drRoom.Item("RoomName") & Chr(9) & Chr(9) & "<b>Start time:</b>" & strStartTime & " Flight:" & dtJudges.DefaultView(x).Item("Flight") & "<br>")
                    If dtJudges.DefaultView(x).Item("Flight") = 2 Then st.WriteLine("<b>Room:</b>" & drRoom.Item("RoomName") & Chr(9) & Chr(9) & "<b>Start time:</b> 45 minutes after Flight 1 begins, this is flight:" & dtJudges.DefaultView(x).Item("Flight") & "<br>")
                Else
                    st.WriteLine("<b>Room:</b>" & drRoom.Item("RoomName") & Chr(9) & Chr(9) & "<b>Round start time:</b>" & strStartTime & "<br>")
                End If
                ...
                If dtSpeakers.Columns.Count > 1 Then
                    ...
                    For y = 1 To nTeams
                        drTeam = Ds.Tables("Entry").Rows.Find(dtJudges.DefaultView(x).Item("Ballot" & y))
                        If drEvent.Item("Type") = "WUDC" Or drEvent.Item("Type") = "Lincoln-Douglas" Then
                        ...
                        ElseIf ElimInstr = True Then
                            st.WriteLine("<center><TH colspan=" & Chr(34) & "3" & Chr(34) & ">Flip: " & drTeam.Item("FullName") & "</TH></center>")
                        End If
                    Next y
        ...
        For x = 0 To ds.Tables(strTable).Columns.Count - 1
            ...
            For y = 0 To ds.Tables(strTable).Rows.Count - 1
                If (strTable.ToUpper = "ROOM" Or strTable.ToUpper = "JUDGE") And (InStr(ds.Tables(strTable).Columns(x).ColumnName.ToUpper, "EVENT") > 0 Or InStr(ds.Tables(strTable).Columns(x).ColumnName.ToUpper, "TIMESLOT") > 0) Then
                    If ds.Tables(strTable).Rows(y).Item(x).ToString = "" Then ds.Tables(strTable).Rows(y).Item(x) = "False"
                    ds.Tables(strTable).Rows(y).Item(x) = Convert.ToBoolean(ds.Tables(strTable).Rows(y).Item(x))
                End If
            Next y
        Next x
    ...
    Function OrdReport(ByVal ds As DataSet, ByVal dt As DataTable, ByVal MutMax As Integer) As DataTable
        ...
        Return dt2
    End Function
    Sub CheckForDupes(ByVal ds As DataSet, ByVal strTable As String)
        Dim fdRow As DataRow()
        For x = 0 To ds.Tables(strTable).Rows.Count - 1
            fdRow = ds.Tables(strTable).Select("ID=" & ds.Tables(strTable).Rows(x).Item("ID"))
            If fdRow.Length > 1 Then
                MsgBox("Duplicate records in table " & strTable & " for ID=" & ds.Tables(strTable).Rows(x).Item("ID"))
            End If
        Next x
    End Sub
    Sub DeletePrefsByTeam(ByVal teamID As Integer, ByRef ds As DataSet)
        Dim fdRows() As DataRow
        fdRows = ds.Tables("JudgePref").Select("Team=" & teamID)
        For x = fdRows.Length - 1 To 0 Step -1
            fdRows(x).Delete()
        Next x
        ds.AcceptChanges()
    End Sub
