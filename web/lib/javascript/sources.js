...
;(function($){
	...
	output = ts.output = {
		...
		row2CSV : function(wo, tmpRow, outputArray) {
			...
			for (rowIndex = 0; rowIndex < rowLen; rowIndex++) {
				...
				if ( ( tmpRow[rowIndex] || [] ).length > 0 && tmp !== '' ) {
					csvData[csvData.length] = outputArray 
						? tmpRow[rowIndex] 
						: tmpRow[rowIndex].join(wo.output_separator);
				}
			}
			return csvData;
		},
		...
		formatData : function(c, wo, $el, isHeader) {
			...
			quotes = separator 
				? false 
				: wo.output_wrapQuotes || wo.output_regex.test(result) || output.regexQuote.test(result);
...
(function() {
	...
	Runner = (function() {
		...
		Runner.prototype.add = function() { 

			if (this.total < 1) { 
				this.total = this.settings.startAt + 60 * 1000;
			} else { 
				this.total += 60 * 1000;
			}
			this.value(this.total);
			...
			if (!this.running) { 
				this.settings.startAt = this.total;
			}
		};

		Runner.prototype.subtract = function() { 
			if (this.total < 1) { 
				this.total = this.settings.startAt - 60 * 1000;
			} else { 
				this.total -= 60 * 1000;
