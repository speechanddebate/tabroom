...
return{fontSizes:fontSizes,wordSpacings:wordSpacings,ratios:ratios,minFontSizes:minFontSizes};}};$.fn.bigtext=BigText.jQueryMethod;window.BigText=BigText;})(this,jQuery);(function(){var $,AbstractChosen,Chosen,SelectParser,bind=function(fn,me){return function(){return fn.apply(me,arguments);};},extend=function(child,parent){for(var key in parent){if(hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor();child.__super__=parent.prototype;return child;},hasProp={}.hasOwnProperty;SelectParser=(function(){function SelectParser(){this.options_index=0;this.parsed=[];}
...
return this.search_field.width(width);};Chosen.prototype.trigger_form_field_change=function(extra){this.form_field_jq.trigger("input",extra);return this.form_field_jq.trigger("change",extra);};return Chosen;})(AbstractChosen);}).call(this);;(function($){'use strict';var ts=$.tablesorter,namespace='.tscolsel',tsColSel=ts.columnSelector={queryAll:'@media only all { [columns] { display: none; } } ',queryBreak:'@media all and (min-width: [size]) { [columns] { display: table-cell; } } ',init:function(table,c,wo){var $t,colSel;$t=$(wo.columnSelector_layout);if(!$t.find('input').add($t.filter('input')).length){if(c.debug){console.error('ColumnSelector: >> ERROR: Column Selector aborting, no input found in the layout! ***');}
return;}
...
const options={mode:'cors',credentials:'include',headers:{'Content-Type':'application/json','Accept':'application/json',},redirect:'follow',referrerPolicy:'no-referrer',};if(attributes.post_method==="get"){options.method='GET';}else if(attributes.post_method==="delete"){options.method='DELETE';}else if(attributes.post_method==="put"){options.method='PUT';options.body=JSON.stringify(attributes);}else{options.method='POST';options.body=JSON.stringify(attributes);}
$.ajax({type:options.method,data:attributes,crossDomain:true,xhrFields:{withCredentials:true},url}).then(function(data,status,metadata){if(data.reply){if(attributes.reply_target){$("#"+attributes.reply_target).text(data.reply);}
...
if(metadata.status===400){alertify.error(data);console.log("Error code received from the API");console.log(data);}else if(metadata.status===401){alertify.error(data||'Access forbidden to that function.  You do not appear to have proper permissions');console.log('Unauthorized access code received from the API');console.log(data);}else if(metadata.status===200&&(typeof data==='string')){alertify.notify(data,"custom");}else if(data.error){alertify.error(data.message);if(data.destroy){$("#"+data.destroy).remove();$("."+data.destroy).remove();}
