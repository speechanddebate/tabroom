...
$.ajax({type:'POST',url:replyUrl,data:{target_id:targetId,property_name:propertyName,setting_name:settingName,property_value:propertyValue,other_value:otherValue,other_text:otherText,related_thing:relatedThing,tourn_id:tournID,parent_id:parentObjectId},success:function(data){if(data.reply){if(replyTarget){$("#"+replyTarget).text(data.reply);}
...
return;}});}
function zebraRows(){var tags=['.row','.lightrow'];var parents=['div','span','table'];tags.forEach(function(Tag){parents.forEach(function(Parent){$(document).find(Tag).parent().closest(Parent).find(".row:visible:even").removeClass("odd");$(document).find(Tag).parent().closest(Parent).find(".row:visible:even").addClass("even");$(document).find(Tag).parent().closest(Parent).find(".row:visible:odd").removeClass("even");$(document).find(Tag).parent().closest(Parent).find(".row:visible:odd").addClass("odd");});});};function uploadName(inputBox){var fileTag=$(inputBox).attr("id");var fileName=$("#"+fileTag).val().replace(/C:\\fakepath\\/i,'');$("."+fileTag).html(fileName);$("."+fileTag).html(fileName);}
