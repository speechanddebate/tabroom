...
function postSwitch(checkObject,replyUrl,callback){var targetId=$(checkObject).attr("target_id");var propertyName=$(checkObject).attr("property_name");var settingName=$(checkObject).attr("setting_name");var relatedThing=$(checkObject).attr("related_thing");var anotherThing=$(checkObject).attr("another_thing");var tournID=$(checkObject).attr("tourn_id");var successAction=$(checkObject).attr("on_success");var replyTarget=$(checkObject).attr("reply_target");var replyAppend=$(checkObject).attr("reply_append");var newParent=$(checkObject).attr("new_parent");var propertyValue=$(checkObject).val();var otherObject=$(checkObject).attr("other_value");var otherOtherObject=$(checkObject).attr("other_other_value");var optionOneId=$(checkObject).attr("option_one");var optionTwoId=$(checkObject).attr("option_two");var postMethod=$(checkObject).attr("post_method");var optionOne;var optionTwo;if(optionOneId){if($("#"+optionOneId).prop("checked")){optionOne=true;}}
if(optionTwoId){if($("#"+optionTwoId).prop("checked")){optionTwo=true;}}
var otherTextId=$(checkObject).attr("other_text");var parentObject=$(checkObject).parent();var parentObjectId=0;if(parentObject){parentObjectId=parentObject.attr('id');}
var otherValue;if(otherObject){otherValue=$("#"+otherObject).val();}
var otherOtherValue;if(otherOtherObject){otherOtherValue=$("#"+otherOtherObject).val();}
...
var accessType="";if(postMethod==="get"){accessType="GET";}else if(postMethod==="delete"){accessType="DELETE";}else if(postMethod==="put"){accessType="PUT";}else{accessType="POST";}
$.ajax({type:accessType,url:replyUrl,data:{target_id:targetId,property_name:propertyName,setting_name:settingName,property_value:propertyValue,other_value:otherValue,option_one:optionOne,option_two:optionTwo,other_text:otherText,related_thing:relatedThing,another_thing:anotherThing,tourn_id:tournID,parent_id:parentObjectId},success:function(data){if(data){if(data.reply){if(replyTarget){$("#"+replyTarget).text(data.reply);}
if(replyAppend){$("#"+replyAppend).append(data.reply);}
$(".replybucket").text(data.reply);$(".replyappend").append(data.reply);}
if(data.error){alertify.error(data.message);if(data.destroy){$("#"+data.destroy).remove();$("."+data.destroy).remove();}
if(data.errSetValue){data.errSetValue.forEach(function(item){$("#"+item.id).val(item.content);});}
if(data.errReplace){data.errReplace.forEach(function(item){if(item.destroy){$("#"+item.id).remove();}else if(item.content){$("#"+item.id).html(item.content);}});}}else if(data.message){alertify.dismissAll();alertify.notify(data.message,"custom");if(data.destroy){$("#"+data.destroy).remove();$("."+data.destroy).remove();}
...
if(data.reprop){data.reprop.forEach(function(item){console.log(item);$("#"+item.id).attr(item.property,item.value);});}
...
function zebraRows(){var tags=['.row','.lightrow'];var parents=['div','span','table'];tags.forEach(function(Tag){parents.forEach(function(Parent){$(document).find(Tag).parent().closest(Parent)
...
.find(".row:visible:odd").addClass("odd");});});};function uploadName(inputBox){var fileTag=$(inputBox).attr("id");var fileName=$("#"+fileTag).val().replace(/C:\\fakepath\\/i,'');$("."+fileTag).html(fileName);$("."+fileTag).html(fileName);}
...
input.form[next_index].focus();}
return true;}
$(document).ready(function(){$('a.login-window').click(function(){var loginBox=$(this).attr('href');$(loginBox).slideDown(300);var popMargTop=($(loginBox).height()+24)/2;var popMargLeft=($(loginBox).width()+24)/2;$(loginBox).css({'margin-top':-popMargTop,'margin-left':-popMargLeft});$('body').append('<div id="mask"></div>');$('#mask').fadeIn(300);$('#username').focus();return false;});$('a.close, #mask').live('click',function(){$('#mask').fadeOut(300,function(){$('#mask').remove();});$('.login-popup').slideUp(300);return false;});$('.hide-menu').click(function(){$('.menu').slideUp(300);$('.content').addClass('nomenu');$('.hide-menu').addClass('hidden');$('.show-menu').removeClass('hidden');});$('.show-menu').click(function(){$('.menu').slideDown(300);$('.content').removeClass('nomenu');$('.show-menu').addClass('hidden');$('.hide-menu').removeClass('hidden');});resizeAll();});var waitForFinalEvent=(function(){var timers={};return function(callback,ms,uniqueId){if(!uniqueId){uniqueId="Don't call this twice without a uniqueId";}
if(timers[uniqueId]){clearTimeout(timers[uniqueId]);}
timers[uniqueId]=setTimeout(callback,ms);};})();function resizeAll(){$('input[type=text]').each(function(){if($(this).parent().is("td")||$(this).parent().is("th")||$(this).parent().is("label")){}else{$(this).width($(this).parent().width()-20);}});$('textarea').each(function(){$(this).width($(this).parent().width()-20);});$('.chosen-container').each(function(){if($(this).parent().is("td")||$(this).parent().is("th")){}else{$(this).width($(this).parent().width()-20);}});}
function fixVisual(){zebraRows();resizeAll();$('table').trigger('applyWidgets');$('table').trigger('update',[true]);console.log("Do it England");}
