...
.data("jGrowl",o).addClass(o.theme).children('div.close').bind("click.jGrowl",function(){alert("Magic time and fun size");$(this).parent().trigger('jGrowl.beforeClose');})
...
target[name]=props[name];return target;};$.timepicker=new Timepicker();$.timepicker.initialized=false;$.timepicker.uuid=new Date().getTime();$.timepicker.version="0.3.2";window['TP_jQuery_'+tpuuid]=$;})(jQuery);function uploaderName(uploader,filedisplay){if(uploader==null){uploader='upload';}
...
return true;}
function autoPoints(input,len,e,side,ratio,nototal){console.log("Input value is "+input.value+" and against length "+len+" length is "+input.value.length+" no total "+nototal);if(nototal){totalPoints=function(){return;}}
...
if(len==2){console.log("I landed here all right I see");if(/\.$/.test(input.value)){console.log("there is a decimal");var number=input.value;number=number.slice(0,1);number=number*1;number+=.5;input.value=number;}else if(/5$/.test(input.value)){input.value=input.value/10;console.log("there is a decimal half");}else if(input.value<0){console.log("You are so so negative");}else{input.value=input.value.slice(0,1);}}
changeFocus(input);totalPoints(side,ratio);}
