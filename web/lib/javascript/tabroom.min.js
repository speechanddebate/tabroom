...
$.ajax({type:'POST',url:replyUrl,data:{target_id:targetId,property_name:propertyName,setting_name:settingName,property_value:propertyValue,other_value:otherValue,other_text:otherText,related_thing:relatedThing,parent_id:parentObjectId},success:function(data){if(data.reply){if(replyTarget){$("#"+replyTarget).text(data.reply);}
if(replyAppend){$("#"+replyAppend).append(data.reply);}
$(".replybucket").text(data.reply);$(".replyappend").append(data.reply);}
if(data.error){alertify.error(data.message);}else if(data.message){alertify.dismissAll();alertify.notify(data.message,"custom");if(data.destroy){$("#"+data.destroy).remove();}
if(successAction==="destroy"){$("#"+targetId).remove();}else if(successAction==="hide"){$("#"+targetId).addClass("hidden");}else if(successAction==="refresh"||successAction==="reload"||data.refresh){window.location.reload();}
