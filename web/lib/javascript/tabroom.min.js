...
if(len==2){if(/\.$/.test(input.value)){var number=input.value;number=number.slice(0,1);number=number*1;number+=.5;input.value=number;}else if(/5$/.test(input.value)){input.value=input.value/10;}else if(input.value<0){}else if(input.value===-5){input.value=-0.5;}else if(input.value==="-."){input.value=-0.5;}else{input.value=input.value.slice(0,1);}}
...
if(input.form[i]==input)index=i;else i++;return index;}
return true;}
function autoSel(input,event){var keyCode=event.keyCode;console.log("Event is "+keyCode);if(keyCode===9){return;}
var next_index=getIndex(input)+1;while(input.form[next_index].tabIndex==-1){next_index=next_index+1;if(next_index>input.form.length)break;}
