...
(function(old){$.fn.attrs=function(){if(arguments.length===0){if(this.length===0){return null;}
var obj={};$.each(this[0].attributes,function(){if(this.specified){obj[this.name]=this.value;}});return obj;}
return old.apply(this,arguments);};})($.fn.attrs);function postSwitch(checkObject,replyUrl,callback,confirmMessage){console.log("Confirm message is "+confirmMessage);if(confirmMessage!=undefined&&confirmMessage!=""){alertify.confirm("Please confirm",confirmMessage,function(e){if(e){}else{return;}},function(no){return;});}
var attributes={};attributes=$(checkObject).attrs();if(attributes.property_value===undefined){attributes.property_value=$(checkObject).attr("value");}
if(attributes.property_value===undefined){attributes.property_value=$(checkObject).val();}
if(checkObject.type==="checkbox"){if($(checkObject).prop("checked")===false){attributes.property_value=0;}}
if(attributes.parent_id===undefined){attributes.parent_id=$(checkObject).parent().attr('id');}
...
var accessType="";if(attributes.post_method==="get"){accessType="GET";}else if(attributes.post_method==="delete"){accessType="DELETE";}else if(attributes.post_method==="put"){accessType="PUT";}else{accessType="POST";}
$.ajax({type:accessType,url:replyUrl,data:attributes,success:function(data,status,object,callback){if(data){if(data.reply){if(attributes.reply_target){$("#"+attributes.reply_target).text(data.reply);}
if(attributes.reply_append){$("#"+replyAppend).append(data.reply);}
...
if(attributes.on_success==="destroy"){$("#"+targetId).remove();}else if(attributes.on_success==="hide"){$("#"+targetId).addClass("hidden");}else if(attributes.on_success==="refresh"||attributes.on_success==="reload"||data.refresh){window.location.reload();}
if(attributes.new_parent){$("#"+targetId).prependTo("#"+attributes.new_parent);}
...
.find(".row:visible:odd").addClass("odd");});});};function fixVisual(){zebraRows();resizeAll();$('table').trigger('applyWidgets');$('table').trigger('update',[true]);}
