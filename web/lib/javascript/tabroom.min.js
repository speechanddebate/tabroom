...
if(option.category_array_index!=null){classes.push("group-option");}
if(option.classes!==""){classes.push(option.classes);}
style=option.style.cssText!==""?" style=\""+option.style+"\"":"";return"<li class=\""+(classes.join(' '))+"\""+style+" data-option-array-index=\""+option.array_index+"\">"+option.search_text+"</li>";};AbstractChosen.prototype.result_add_category=function(group){if(!(group.search_match||group.category_match)){return'';}
...
this.result_clear_highlight();this.result_single_selected=null;this.results_build();if(this.results_showing){return this.winnow_results();}};AbstractChosen.prototype.results_toggle=function(){if(this.results_showing){return this.results_hide();}else{return this.results_show();}};AbstractChosen.prototype.results_search=function(evt){if(this.results_showing){return this.winnow_results();}else{return this.results_show();}};AbstractChosen.prototype.winnow_results=function(){var escapedSearchText,option,regex,regexAnchor,results,results_category,searchText,startpos,text,zregex,_i,_len,_ref;this.no_results_clear();results=0;searchText=this.get_search_text();escapedSearchText=searchText.replace(/[-[\]{}()*+?.,\\^$|#\s]/g,"\\$&");regexAnchor=this.search_contains?"":"^";regex=new RegExp(regexAnchor+escapedSearchText,'i');zregex=new RegExp(escapedSearchText,'i');_ref=this.results_data;for(_i=0,_len=_ref.length;_i<_len;_i++){option=_ref[_i];option.search_match=false;results_category=null;if(this.include_option_in_results(option)){if(option.group){option.category_match=false;option.active_options=0;}
if((option.category_array_index!=null)&&this.results_data[option.category_array_index]){results_category=this.results_data[option.category_array_index];if(results_category.active_options===0&&results_category.search_match){results+=1;}
results_category.active_options+=1;}
if(!(option.group&&!this.category_search)){option.search_text=option.group?option.label:option.html;option.search_match=this.search_string_match(option.search_text,regex);if(option.search_match&&!option.group){results+=1;}
if(option.search_match){if(searchText.length){startpos=option.search_text.search(zregex);text=option.search_text.substr(0,startpos+searchText.length)+'</em>'+option.search_text.substr(startpos+searchText.length);option.search_text=text.substr(0,startpos)+'<em>'+text.substr(startpos);}
if(results_category!=null){results_category.category_match=true;}}else if((option.category_array_index!=null)&&this.results_data[option.category_array_index].search_match){option.search_match=true;}}}}
...
.attr('role','grid');c.$headers=$table.find(c.selectorHeaders);if(!c.namespace){c.namespace='.tablesorter'+Math.random().toString(16).slice(2);}else{c.namespace='.'+c.namespace.replace(ts.regex.nonWord,'');}
...
.bind('sortReset'+namespace,function(e,callback){e.stopPropagation();ts.sortReset(this.config,callback);})
...
.bind('applyWidgets'+namespace,function(e,init){e.stopPropagation();ts.applyWidget(this,init);})
...
$cell=$.fn.closest?$(this).closest('th, td'):/TH|TD/.test(this.nodeName)?$(this):$(this).parents('th, td');temp=$headers.index($cell);c.last.clickedIndex=(temp<0)?$cell.attr('data-column'):temp;cell=c.$headers[c.last.clickedIndex];if(cell&&!cell.sortDisabled){ts.initSort(c,cell,e);}});if(c.cancelSelection){$headers
.attr('unselectable','on')
.bind('selectstart',false)
.css({'user-select':'none','MozUserSelect':'none'});}},buildHeaders:function(c){var $temp,icon,timer,indx;c.headerList=[];c.headerContent=[];c.sortVars=[];if(c.debug){timer=new Date();}
...
cacheIndex++;}
continue;}
rowData.$row=$row;rowData.order=rowIndex;cacheIndex=0;max=c.columns;for(colIndex=0;colIndex<max;++colIndex){cell=$row[0].cells[colIndex];if(cell&&cacheIndex<c.columns){hasParser=typeof parsers[cacheIndex]!=='undefined';if(!hasParser&&c.debug){console.warn('No parser found for row: '+rowIndex+', column: '+colIndex+
...
if(c.debug){len=Math.min(5,c.cache[0].normalized.length);console[console.group?'group':'log']('Building cache for '+c.totalRows+
...
return data;}},setHeadersCss:function(c){var $sorted,indx,column,list=c.sortList,len=list.length,none=ts.css.sortNone+' '+c.cssNone,css=[ts.css.sortAsc+' '+c.cssAsc,ts.css.sortDesc+' '+c.cssDesc],cssIcon=[c.cssIconAsc,c.cssIconDesc,c.cssIconNone],aria=['ascending','descending'],$headers=c.$table
.find('tfoot tr')
.children('td, th')
.add($(c.namespace+'_extra_headers'))
.removeClass(css.join(' '));c.$headers
...
.addClass(cssIcon[2]);for(indx=0;indx<len;indx++){if(list[indx][1]!==2){$sorted=c.$headers.filter(function(i){var include=true,$el=c.$headers.eq(i),col=parseInt($el.attr('data-column'),10),end=col+c.$headers[i].colSpan;for(;col<end;col++){include=include?include||ts.isValueInArray(col,c.sortList)>-1:false;}
return include;});$sorted=$sorted
.not('.sorter-false')
.filter('[data-column="'+list[indx][0]+'"]'+(len===1?':last':''));if($sorted.length){for(column=0;column<$sorted.length;column++){if(!$sorted[column].sortDisabled){$sorted
.eq(column)
.removeClass(none)
...
dir=order[c.sortVars[col].count];if(dir<2){c.sortList[c.sortList.length]=[col,dir];if(cell.colSpan>1){for(indx=1;indx<cell.colSpan;indx++){c.sortList[c.sortList.length]=[col+indx,dir];c.sortVars[col+indx].count=$.inArray(dir,order);}}}
...
' widget'+(widget!==1?'s':'')+ts.benchmark(time));}},removeWidget:function(table,name,refreshing){table=$(table)[0];var index,widget,indx,len,c=table.config;if(name===true){name=[];len=ts.widgets.length;for(indx=0;indx<len;indx++){widget=ts.widgets[indx];if(widget&&widget.id){name[name.length]=widget.id;}}}else{name=($.isArray(name)?name.join(','):name||'').toLowerCase().split(/[\s,]+/);}
...
.find(c.selectorSort)
.unbind(('mousedown mouseup keypress '.split(' ').join(c.namespace+' ')).replace(ts.regex.spaces,' '));ts.restoreHeaders(table);$t.toggleClass(ts.css.table+' '+c.tableClass+' tablesorter-'+c.theme,removeClasses===false);table.hasInitialized=false;delete table.config.cache;if(typeof callback==='function'){callback(table);}
...
var tbodyIndex,$tbody,$tbodies=c.$tbodies,toRemove=(wo.zebra||['even','odd']).join(' ');for(tbodyIndex=0;tbodyIndex<$tbodies.length;tbodyIndex++){$tbody=ts.processTbody(table,$tbodies.eq(tbodyIndex),true);$tbody.children().removeClass(toRemove);ts.processTbody(table,$tbody,false);}}});})(jQuery);return jQuery.tablesorter;}));(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else if(typeof module==='object'&&typeof module.exports==='object'){module.exports=factory(require('jquery'));}else{factory(jQuery);}}(function(jQuery){;(function($,window,document){'use strict';var ts=$.tablesorter||{};ts.storage=function(table,key,value,options){table=$(table)[0];var cookieIndex,cookies,date,hasStorage=false,values={},c=table.config,wo=c&&c.widgetOptions,storageType=(options&&options.useSessionStorage)||(wo&&wo.storage_useSessionStorage)?'sessionStorage':'localStorage',$table=$(table),id=options&&options.id||$table.attr(options&&options.group||wo&&wo.storage_group||'data-table-group')||wo&&wo.storage_tableId||table.id||$('.tablesorter').index($table),url=options&&options.url||$table.attr(options&&options.page||wo&&wo.storage_page||'data-table-page')||wo&&wo.storage_fixedUrl||c&&c.fixedUrl||window.location.pathname;$.extend(true,ts.defaults,{fixedUrl:'',widgetOptions:{storage_fixedUrl:'',storage_group:'',storage_page:'',storage_tableId:'',storage_useSessionStorage:''}});if(storageType in window){try{window[storageType].setItem('_tmptest','temp');hasStorage=true;window[storageType].removeItem('_tmptest');}catch(error){if(c&&c.debug){console.warn(storageType+' is not supported in this browser');}}}
...
if(c.showProcessing){txt='filterStart filterEnd '.split(' ').join(c.namespace+'filter ');c.$table
...
$(this).attr('data-lastSearchTime',new Date().getTime());if(event.which===tskeyCodes.escape){this.value=wo.filter_resetOnEsc?'':c.lastSearch[column];}else if(liveSearch===false){return;}else if(this.value!==''&&((typeof liveSearch==='number'&&this.value.length<liveSearch)||(event.which!==tskeyCodes.enter&&event.which!==tskeyCodes.backSpace&&(event.which<tskeyCodes.space||(event.which>=tskeyCodes.left&&event.which<=tskeyCodes.down))))){return;}
tsf.searching(table,true,true,column);})
.bind('search change keypress input '.split(' ').join(namespace+' '),function(event){var column=parseInt($(this).attr('data-column'),10),liveSearch=typeof wo.filter_liveSearch==='boolean'?wo.filter_liveSearch:ts.getColumnData(table,wo.filter_liveSearch,column);if(table.config.widgetOptions.filter_initialized&&(event.which===tskeyCodes.enter||event.type==='search'||(event.type==='change'||event.type==='input')&&liveSearch===true&&this.value!==c.lastSearch[column])){event.preventDefault();$(this).attr('data-lastSearchTime',new Date().getTime());tsf.searching(table,event.type!=='keypress',true,column);}});},searching:function(table,filter,skipFirst,column){var liveSearch,wo=table.config.widgetOptions;if(typeof column==='undefined'){liveSearch=false;}else{liveSearch=typeof wo.filter_liveSearch==='boolean'?wo.filter_liveSearch:ts.getColumnData(table,wo.filter_liveSearch,column);if(typeof liveSearch==='undefined'){liveSearch=wo.filter_liveSearch.fallback||false;}}
clearTimeout(wo.filter_searchTimer);if(typeof filter==='undefined'||filter===true){wo.filter_searchTimer=setTimeout(function(){tsf.checkFilters(table,filter,skipFirst);},liveSearch?wo.filter_searchDelay:10);}else{tsf.checkFilters(table,filter,skipFirst);}},checkFilters:function(table,filter,skipFirst){var c=table.config,wo=c.widgetOptions,filterArray=$.isArray(filter),filters=(filterArray)?filter:ts.getFilters(table,true),combinedFilters=(filters||[]).join('');if($.isEmptyObject(c.cache)){if(c.delayInit&&(!c.pager||c.pager&&c.pager.initialized)){ts.updateCache(c,function(){tsf.checkFilters(table,false,skipFirst);});}
return;}
...
if(c.showProcessing){setTimeout(function(){tsf.findRows(table,filters,combinedFilters);return false;},30);}else{tsf.findRows(table,filters,combinedFilters);return false;}},hideFiltersCheck:function(c){if(typeof c.widgetOptions.filter_hideFilters==='function'){var val=c.widgetOptions.filter_hideFilters(c);if(typeof val==='boolean'){return val;}}
...
c.lastCombinedFilter=combinedFilters;c.lastSearch=storedFilters;c.$table.data('lastSearch',storedFilters);if(wo.filter_saveFilters&&ts.storage){ts.storage(table,'tablesorter-filters',tsf.processFilters(storedFilters,true));}
...
.find('thead')
.find('select.'+tscss.filter+'[data-column="'+column+'"]')
.val();if(typeof arry==='undefined'||arry===''){arry=tsf.getOptionSource(table,column,onlyAvail);}
...
checkSizes();},options.timer);};ts.addWidget({id:'stickyHeaders',priority:54,options:{stickyHeaders:'',stickyHeaders_appendTo:null,stickyHeaders_attachTo:null,stickyHeaders_xScroll:null,stickyHeaders_yScroll:null,stickyHeaders_offset:0,stickyHeaders_filteredToTop:true,stickyHeaders_cloneId:'-sticky',stickyHeaders_addResizeEvent:true,stickyHeaders_includeCaption:true,stickyHeaders_zIndex:2},format:function(table,c,wo){if(c.$table.hasClass('hasStickyHeaders')||($.inArray('filter',c.widgets)>=0&&!c.$table.hasClass('hasFilters'))){return;}
...
.css(cssSettings);if(isVisible!==laststate||resizing){resizeHeader();laststate=isVisible;}};if($attach.length&&!$attach.css('position')){$attach.css('position','relative');}
...
$table.triggerHandler('stickyHeadersInit');},remove:function(table,c,wo){var namespace=c.namespace+'stickyheaders ';c.$table
...
.bind('columnUpdate'+namespace+' pagerComplete'+namespace,function(){ts.resizable.setHandlePosition(c,wo);})
.find('thead:first')
...
opts.cssBefore.clip=opts.cssBefore.clip||clip||'rect(0px 0px 0px 0px)';var d=opts.cssBefore.clip.match(/(\d+)/g);var t=parseInt(d[0],10),r=parseInt(d[1],10),b=parseInt(d[2],10),l=parseInt(d[3],10);opts.before.push(function(curr,next,opts){if(curr==next)return;var $curr=$(curr),$next=$(next);$.fn.cycle.commonReset(curr,next,opts,true,true,false);opts.cssAfter.display='block';var step=1,count=parseInt((opts.speedIn/13),10)-1;(function f(){var tt=t?t-parseInt(step*(t/count),10):0;var ll=l?l-parseInt(step*(l/count),10):0;var bb=b<h?b+parseInt(step*((h-b)/count||1),10):h;var rr=r<w?r+parseInt(step*((w-r)/count||1),10):w;$next.css({clip:'rect('+tt+'px '+rr+'px '+bb+'px '+ll+'px)'});(step++<=count)?setTimeout(f,13):$curr.css('display','none');})();});$.extend(opts.cssBefore,{display:'block',opacity:1,top:0,left:0});opts.animIn={left:0};opts.animOut={left:0};};})(jQuery);(function($){var $ie6=(function(){return false===$.support.boxModel&&$.support.objectAll&&$.support.leadingWhitespace;})();$.jGrowl=function(m,o){if($('#jGrowl').size()==0)
...
this.notifications.push({message:message,options:o});o.log.apply(this.element,[this.element,message,o]);},render:function(notification){var self=this;var message=notification.message;var o=notification.options;var notification=$('<div/>')
...
.data("jGrowl",o).addClass(o.theme).children('div.close').bind("click.jGrowl",function(){$(this).parent().trigger('jGrowl.beforeClose');})
.parent();$(notification).bind("mouseover.jGrowl",function(){$('div.jGrowl-notification',self.element).data("jGrowl.pause",true);}).bind("click.jGrowl",function(){$(this).trigger('jGrowl.beforeClose');}).bind("mouseout.jGrowl",function(){$('div.jGrowl-notification',self.element).data("jGrowl.pause",false);}).bind('jGrowl.beforeOpen',function(){if(o.beforeOpen.apply(notification,[notification,message,o,self.element])!=false){$(this).trigger('jGrowl.open');}}).bind('jGrowl.open',function(){if(o.open.apply(notification,[notification,message,o,self.element])!=false){if(o.glue=='after'){$('div.jGrowl-notification:last',self.element).after(notification);}else{$('div.jGrowl-notification:first',self.element).before(notification);}
...
target[name]=props[name];return target;};$.timepicker=new Timepicker();$.timepicker.initialized=false;$.timepicker.uuid=new Date().getTime();$.timepicker.version="0.3.2";window['TP_jQuery_'+tpuuid]=$;})(jQuery);function postSwitch(checkObject,replyUrl){var targetId=$("#"+checkObject.id).attr("target_id");var propertyName=$("#"+checkObject.id).attr("property_name");var settingName=$("#"+checkObject.id).attr("setting_name");var propertyValue=checkObject.value;if(propertyValue===undefined){propertyValue=$("#"+checkObject.id).attr("value");}
if(checkObject.type==="checkbox"){if($("#"+checkObject.id).prop("checked")===false){propertyValue=0;}}
$.ajax({type:'POST',url:replyUrl,data:{target_id:targetId,property_name:propertyName,setting_name:settingName,property_value:propertyValue},success:function(data){$('table').trigger('applyWidgets');if(data.error){$.jGrowl(data.message,{header:'Oops!',life:4000,position:'top-center',theme:"warning"});}else{$.jGrowl(data.message,{header:'Result',life:1000,position:'top-center'});}}});}
function zebraRows(){$(".main").find(".row:even").addClass("even");$(".main").find(".row:odd").addClass("odd");$(".blankfull").find(".row:even").addClass("even");$(".blankfull").find(".row:odd").addClass("odd");$(".menu").find(".row:even").addClass("even");$(".menu").find(".row:odd").addClass("odd");$(".sidenote").find(".row:even").addClass("even");$(".sidenote").find(".row:odd").addClass("odd");$("table").find(".row:even").removeClass("odd");$("table").find(".row:even").addClass("even");$("table").find(".row:odd").removeClass("even");$("table").find(".row:odd").addClass("odd");$(".pagehalf").find(".row:even").addClass("even");$(".pagehalf").find(".row:even").removeClass("odd");$(".pagehalf").find(".row:odd").addClass("odd");$(".pagehalf").find(".row:odd").removeClass("even");$(".main").find(".lightrow:even").addClass("lighteven");$(".main").find(".lightrow:even").removeClass("lightodd");$(".main").find(".lightrow:odd").addClass("lightodd");$(".main").find(".lightrow:odd").removeClass("lighteven");$(".blankfull").find(".lightrow:even").addClass("lighteven");$(".blankfull").find(".lightrow:even").removeClass("lightodd");$(".blankfull").find(".lightrow:odd").addClass("lightodd");$(".blankfull").find(".lightrow:odd").removeClass("lighteven");$(".menu").find(".lightrow:even").addClass("lighteven");$(".menu").find(".lightrow:odd").addClass("lightodd");$(".sidenote").find(".lightrow:even").removeClass("lightodd");$(".sidenote").find(".lightrow:even").addClass("lighteven");$(".sidenote").find(".lightrow:odd").removeClass("lighteven");$(".sidenote").find(".lightrow:odd").addClass("lightodd");$("table").find(".lightrow:even").removeClass("lightodd");$("table").find(".lightrow:even").addClass("lighteven");$("table").find(".lightrow:odd").removeClass("lighteven");$("table").find(".lightrow:odd").addClass("lightodd");$(".pagehalf").find(".lightrow:even").addClass("lighteven");$(".pagehalf").find(".lightrow:even").removeClass("lightodd");$(".pagehalf").find(".lightrow:odd").addClass("lightodd");$(".pagehalf").find(".lightrow:odd").removeClass("lighteven");};function uploaderName(uploader,filedisplay){if(uploader==null){uploader='upload';}
...
if(!containsElement(filter,keyCode)){if(input.value=="A"||input.value=="a"||input.value==1||input.value=="p"||input.value=="P"||input.value=="g"||input.value=="G"){input.value=aff;$('.aff').show();$('.neg').hide();var winner=document.getElementById("winner");winner.value=affid;lowPointWin(1);changeFocus(input);}
if(input.value=="N"||input.value=="n"||input.value==3||input.value=="c"||input.value=="C"||input.value=="o"||input.value=="O"){input.value=neg;$('.neg').show();$('.aff').hide();var winner=document.getElementById("winner");winner.value=negid;lowPointWin(2);changeFocus(input);}}
...
if(len==2){if(step==1){}else{if(/\.$/.test(input.value)){var number=input.value;number=number.slice(0,1);number=number*1;number+=.5;input.value=number;}else if(/5$/.test(input.value)){input.value=input.value/10;}else if(input.value<0){}else if(input.value===-5){input.value=-0.5;}else if(input.value==="-."){input.value=-0.5;}else{input.value=input.value.slice(0,1);}}}
...
if(input.form[i]==input)index=i;else i++;return index;}
return true;}
function autoSel(input,event){var keyCode=event.keyCode;if(keyCode===9){return;}
var next_index=getIndex(input)+1;while(input.form[next_index].tabIndex==-1){next_index=next_index+1;if(next_index>input.form.length)break;}
input.form[next_index].focus();function getIndex(input){var index=-1,i=0,found=false;while(i<input.form.length&&index==-1)
if(input.form[i]==input)index=i;else i++;return index;}}
function autoTab(input,len,e){var keyCode=e.keyCode;var filter=[0,8,9,16,17,18,37,38,39,40,46];if(len==9&&input.value.length>=2&&!containsElement(filter,keyCode)){if(input.value!=30){var number=input.value;number=number*1;number=number/10;number=number+20;input.value=number;}
...
if(input.form[i]==input)index=i;else i++;return index;}
return true;}
$(document).ready(function(){$('a.login-window').click(function(){var loginBox=$(this).attr('href');$(loginBox).slideDown(300);var popMargTop=($(loginBox).height()+24)/2;var popMargLeft=($(loginBox).width()+24)/2;$(loginBox).css({'margin-top':-popMargTop,'margin-left':-popMargLeft});$('body').append('<div id="mask"></div>');$('#mask').fadeIn(300);return false;});$('a.close, #mask').live('click',function(){$('#mask').fadeOut(300,function(){$('#mask').remove();});$('.login-popup').slideUp(300);return false;});});$(document).ready(function(){$('.hide-menu').click(function(){$('.menu').slideUp(300);$('.content').addClass('nomenu');$('.hide-menu').addClass('hidden');$('.show-menu').removeClass('hidden');});});$(document).ready(function(){$('.show-menu').click(function(){$('.menu').slideDown(300);$('.content').removeClass('nomenu');$('.show-menu').addClass('hidden');$('.hide-menu').removeClass('hidden');});});
