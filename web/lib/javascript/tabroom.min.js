...
return;}});}
function zebraRows(){var elements=['span','div'];var tags=['.row','.lightrow'];tags.forEach(function(tag){elements.forEach(function(el){$(document).find(tag).closest(el).find(".row:even").removeClass("odd");$(document).find(tag).closest(el).find(".row:even").addClass("even");$(document).find(tag).closest(el).find(".row:odd").removeClass("even");$(document).find(tag).closest(el).find(".row:odd").addClass("odd");});});};function uploadName(inputBox){var fileTag=$(inputBox).attr("id");var fileName=$("#"+fileTag).val().replace(/C:\\fakepath\\/i,'');$("."+fileTag).html(fileName);$("."+fileTag).html(fileName);}
