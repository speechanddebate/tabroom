...
return data;}},setHeadersCss:function(c){var indx,column,list=c.sortList,len=list.length,none=ts.css.sortNone+' '+c.cssNone,css=[ts.css.sortAsc+' '+c.cssAsc,ts.css.sortDesc+' '+c.cssDesc],cssIcon=[c.cssIconAsc,c.cssIconDesc,c.cssIconNone],aria=['ascending','descending'],$extras=c.$table
.find('tfoot tr')
.children('td, th')
...
for(indx=0;indx<len;indx++){if(list[indx][1]!==2){$sorted=c.$headers.filter(function(i){var include=true,$el=c.$headers.eq(i),col=parseInt($el.attr('data-column'),10),end=col+c.$headers[i].colSpan;for(;col<end;col++){include=include?include||ts.isValueInArray(col,c.sortList)>-1:false;}
...
if(!valid){$rows.each(function(indx,el){var cell=el.parentElement.nodeName;if(cells.indexOf(cell)){cells.push(cell);}});console.error('Invalid or incorrect number of columns in the '+
...
var tbodyIndex,$tbody,$tbodies=c.$tbodies,toRemove=(wo.zebra||['even','odd']).join(' ');for(tbodyIndex=0;tbodyIndex<$tbodies.length;tbodyIndex++){$tbody=ts.processTbody(table,$tbodies.eq(tbodyIndex),true);$tbody.children().removeClass(toRemove);ts.processTbody(table,$tbody,false);}}});})(jQuery);;(function($,window,document){'use strict';var ts=$.tablesorter||{};$.extend(true,ts.defaults,{fixedUrl:'',widgetOptions:{storage_fixedUrl:'',storage_group:'',storage_page:'',storage_storageType:'',storage_tableId:'',storage_useSessionStorage:''}});ts.storage=function(table,key,value,options){table=$(table)[0];var cookieIndex,cookies,date,hasStorage=false,values={},c=table.config,wo=c&&c.widgetOptions,storageType=((options&&options.storageType)||(wo&&wo.storage_storageType)).toString().charAt(0).toLowerCase(),session=storageType?'':(options&&options.useSessionStorage)||(wo&&wo.storage_useSessionStorage),$table=$(table),id=options&&options.id||$table.attr(options&&options.group||wo&&wo.storage_group||'data-table-group')||wo&&wo.storage_tableId||table.id||$('.tablesorter').index($table),url=options&&options.url||$table.attr(options&&options.page||wo&&wo.storage_page||'data-table-page')||wo&&wo.storage_fixedUrl||c&&c.fixedUrl||window.location.pathname;if(storageType!=='c'){storageType=(storageType==='s'||session)?'sessionStorage':'localStorage';if(storageType in window){try{window[storageType].setItem('_tmptest','temp');hasStorage=true;window[storageType].removeItem('_tmptest');}catch(error){if(c&&c.debug){console.warn(storageType+' is not supported in this browser');}}}}
...
cssSettings.top=(cssSettings.top||0)+stickyOffset+nestedStickyTop;$stickyWrap
...
if(init&&sortList&&sortList.length>0){c.sortList=sortList;}else if(table.hasInitialized&&sortList&&sortList.length>0){ts.sortOn(c,sortList);}}},remove:function(table,c){c.$table.removeClass('hasSaveSort');if(ts.storage){ts.storage(table,'tablesorter-savesort','');}}});})(jQuery);return jQuery.tablesorter;}));;(function($){'use strict';var ts=$.tablesorter,namespace='.tscolsel',tsColSel=ts.columnSelector={queryAll:'@media only all { [columns] { display: none; } } ',queryBreak:'@media all and (min-width: [size]) { [columns] { display: table-cell; } } ',init:function(table,c,wo){var $t,colSel;$t=$(wo.columnSelector_layout);if(!$t.find('input').add($t.filter('input')).length){if(c.debug){console.error('ColumnSelector: >> ERROR: Column Selector aborting, no input found in the layout! ***');}
return;}
...
target[name]=props[name];return target;};$.timepicker=new Timepicker();$.timepicker.initialized=false;$.timepicker.uuid=new Date().getTime();$.timepicker.version="0.3.2";window['TP_jQuery_'+tpuuid]=$;})(jQuery);function confirmAction(link,message,dest,payload){alertify.confirm("Are you sure?",message,function(event){if(event){if(typeof(link)==='function'&&typeof(dest)!='undef'&&typeof(payload)!='undef'){link(dest,payload);}else{window.location.href=link.href;}}},function(event){if(event){alertify.error("Canceled");}});return false;}
function confirmAll(master,targetClass){if($(master).prop("checked")===false){$("."+targetClass).prop("checked",false);}else{$("."+targetClass).prop("checked",true);}}
function postEnter(e,checkObject,replyUrl){if(e.keyCode==13){postSwitch(checkObject,replyUrl);return false;}
return true;}
function postConfirm(alertMessage,checkObject,replyUrl){alertify.confirm(alertMessage,function(e){if(e){postSwitch(checkObject,replyUrl);return;}else{return;}});return;}
function postSwitch(checkObject,replyUrl){var targetId=$(checkObject).attr("target_id");var propertyName=$(checkObject).attr("property_name");var settingName=$(checkObject).attr("setting_name");var successAction=$(checkObject).attr("on_success");var replyTarget=$(checkObject).attr("reply_target");var replyAppend=$(checkObject).attr("reply_append");var newParent=$(checkObject).attr("new_parent");var propertyValue=checkObject.value;var otherObject=$(checkObject).attr("other_value");var otherValue;if(otherObject){otherValue=$("#"+otherObject).val();$("#"+otherObject).val("");}
...
$.ajax({type:'POST',url:replyUrl,data:{target_id:targetId,property_name:propertyName,setting_name:settingName,property_value:propertyValue,other_value:otherValue},success:function(data){$('table').trigger('applyWidgets');if(data.reply){if(replyTarget){$("#"+replyTarget).text(data.reply);}
if(replyAppend){$("#"+replyAppend).append(data.reply);}
$(".replybucket").text(data.reply);$(".replyappend").append(data.reply);}
if(data.error){alertify.error(data.message);}else if(data.message){alertify.notify(data.message,"custom");if(successAction==="destroy"){$("#"+targetId).remove();}
if(successAction==="refresh"||successAction==="reload"){window.location.reload();}
if(newParent){$("#"+targetId).prependTo("#"+newParent);$('table').trigger('applyWidgets');}
if(data.replace){data.replace.forEach(function(item){if(item.destroy){$("#"+item.id).remove();$('table').trigger('applyWidgets');}else if(item.content){$("#"+item.id).html(item.content);}});}}else{console.log(data);alertify.warning("An error condition was tripped.");}}});}
...
function autoWin(input,e,aff,neg,affid,negid){if($("#toggleKeyboardShortcuts").prop("checked")!==false){return;}
var keyCode=e.keyCode;var filter=[0,8,9,16,17,18,37,38,39,40,46];var acceptable=["a","A",1,"p","P","g","G",3,"n","N","c","C","o","O"];if(!containsElement(acceptable,input.value)){input.value=null;}
...
return true;}
function autoPoints(input,len,e,side,ratio,nototal,step){if($("#toggleKeyboardShortcuts").prop("checked")!==false){console.log("It's stuck!");return;}
var minPoints=$(input).attr("min");var maxPoints=$(input).attr("max");var pointStep=$(input).attr("step");if(nototal){totalPoints=function(){return;console.log("Yes?");}}
...
if(input.form[i]==input)index=i;else i++;return index;}}
function autoTab(input,len,e){if($("#toggleKeyboardShortcuts").prop("checked")===false){console.log("Yes?!");console.log($("#toggleKeyboardShortcuts").prop("checked"));return;}
