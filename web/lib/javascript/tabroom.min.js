...
function postSwitch(checkObject,replyUrl){var targetId=$(checkObject).attr("target_id");var propertyName=$(checkObject).attr("property_name");var settingName=$(checkObject).attr("setting_name");var successAction=$(checkObject).attr("on_success");console.log("Target "+targetId+" and property "+propertyName);var propertyValue=checkObject.value;if(propertyValue===undefined){propertyValue=$(checkObject).attr("value");}
if(checkObject.type==="checkbox"){if($(checkObject).prop("checked")===false){propertyValue=0;}}
$.ajax({type:'POST',url:replyUrl,data:{target_id:targetId,property_name:propertyName,setting_name:settingName,property_value:propertyValue},success:function(data){if(successAction==="destroy"){$("#"+targetId).remove();}
$('table').trigger('applyWidgets');if(data.reply){$(".replybucket").text(data.reply);}
