...
return;}});}
function zebraRows(){var tags=['.row','.lightrow'];var parents=['div','span','table'];tags.forEach(function(Tag){parents.forEach(function(Parent){$(document).find(Tag).parent().closest(Parent).find(".row:even").removeClass("odd");$(document).find(Tag).parent().closest(Parent).find(".row:even").addClass("even");$(document).find(Tag).parent().closest(Parent).find(".row:odd").removeClass("even");$(document).find(Tag).parent().closest(Parent).find(".row:odd").addClass("odd");});});};function uploadName(inputBox){var fileTag=$(inputBox).attr("id");var fileName=$("#"+fileTag).val().replace(/C:\\fakepath\\/i,'');$("."+fileTag).html(fileName);$("."+fileTag).html(fileName);}
