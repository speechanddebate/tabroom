...
$.ajax({type:'POST',url:replyUrl,data:{target_id:targetId,property_name:propertyName,setting_name:settingName,property_value:propertyValue,other_value:otherValue,parent_id:parentObjectId},success:function(data){console.log("Data");console.log(data);if(data.reply){if(replyTarget){$("#"+replyTarget).text(data.reply);}
if(replyAppend){$("#"+replyAppend).append(data.reply);}
$(".replybucket").text(data.reply);$(".replyappend").append(data.reply);}
if(data.error){alertify.error(data.message);}else if(data.message){alertify.dismissAll();alertify.notify(data.message,"custom");if(successAction==="destroy"){$("#"+targetId).remove();}
if(data.destroy){$("#"+data.destroy).remove();}
if(successAction==="refresh"||successAction==="reload"||data.refresh){window.location.reload();}
if(newParent){$("#"+targetId).prependTo("#"+newParent);}
if(data.newParent){$("#"+targetId).prependTo("#"+data.newParent);}
console.log("Data replace:");console.log(data.replace);if(data.replace){data.replace.forEach(function(item){if(item.destroy){$("#"+item.id).remove();}else if(item.content){console.log("replacing ID "+item.id+" with content");console.log(item.content);$("#"+item.id).html(item.content);}});}
if(data.norefresh){}else{$('table').trigger('applyWidgets');$('table').trigger('update',[true]);}}else{console.log(data);alertify.warning("An error condition was tripped.");}
...
if(len==2){if(step==1){}else{if(/\.$/.test(input.value)){var number=input.value;number=number.slice(0,1);number=number*1;number+=.5;input.value=number;}else if(/5$/.test(input.value)){input.value=input.value/10;}else if(input.value<0){}else if(input.value===-5){input.value=-0.5;}else if(input.value==="-."){input.value=-0.5;}else{}}}
