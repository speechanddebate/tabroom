...
return this.search_field.width(width);};Chosen.prototype.trigger_form_field_change=function(extra){this.form_field_jq.trigger("input",extra);return this.form_field_jq.trigger("change",extra);};return Chosen;})(AbstractChosen);}).call(this);(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else if(typeof module==='object'&&typeof module.exports==='object'){module.exports=factory(require('jquery'));}else{factory(jQuery);}}(function(jQuery){;(function($){'use strict';var ts=$.tablesorter={version:'2.31.0',parsers:[],widgets:[],defaults:{theme:'default',widthFixed:false,showProcessing:false,headerTemplate:'{content}',onRenderTemplate:null,onRenderHeader:null,cancelSelection:true,tabIndex:true,dateFormat:'mmddyyyy',sortMultiSortKey:'shiftKey',sortResetKey:'ctrlKey',usNumberFormat:true,delayInit:false,serverSideSorting:false,resort:true,headers:{},ignoreCase:true,sortForce:null,sortList:[],sortAppend:null,sortStable:false,sortInitialOrder:'asc',sortLocaleCompare:false,sortReset:false,sortRestart:false,emptyTo:'bottom',stringTo:'max',duplicateSpan:true,textExtraction:'basic',textAttribute:'data-text',textSorter:null,numberSorter:null,initWidgets:true,widgetClass:'widget-{name}',widgets:[],widgetOptions:{zebra:['even','odd']},initialized:null,tableClass:'',cssAsc:'',cssDesc:'',cssNone:'',cssHeader:'',cssHeaderRow:'',cssProcessing:'',cssChildRow:'tablesorter-childRow',cssInfoBlock:'tablesorter-infoOnly',cssNoSort:'tablesorter-noSort',cssIgnoreRow:'tablesorter-ignoreRow',cssIcon:'tablesorter-icon',cssIconNone:'',cssIconAsc:'',cssIconDesc:'',cssIconDisabled:'',pointerClick:'click',pointerDown:'mousedown',pointerUp:'mouseup',selectorHeaders:'> thead th, > thead td',selectorSort:'th, td',selectorRemove:'.remove-me',debug:false,headerList:[],empties:{},strings:{},parsers:[],globalize:0,imgAttr:0
...
return _;};}));$('.noenter').on('keyup keypress',function(e){var keyCode=e.keyCode||e.which;if(keyCode===13){e.preventDefault();return false;}});function copyToClipboard(elementId,textLabel){var $temp=$("<input>");$("body").append($temp);$temp.val($("#"+elementId).text()).select();document.execCommand("copy");$temp.remove();if(!textLabel){textLabel="Text ";}
...
function postValue(value,url,callback){var data={'value':value};$.ajax({type:'POST',url,data,success:function(data,status,object,newCallback){if(data){if(data.reply){if(data.reply_target){$("#"+data.reply_target).text(data.reply);}
if(data.reply_append){$("#"+data.reply_append).append(data.reply);}
$(".replybucket").text(data.reply);$(".replyappend").append(data.reply);}
if(data.error){alertify.error(data.message);console.log(data);if(data.destroy){$("#"+data.destroy).remove();$("."+data.destroy).remove();}
if(data.errSetValue){data.errSetValue.forEach(function(item){$("#"+item.id).val(item.content);});}
...
if(sizeLimit>0&&typeof FileReader!=="undefined"){var size=document.getElementById(uploader).files[0].size;if(size>(parseInt(sizeLimit)*1024*1024)){document.getElementById(uploader).value="";var sizeString=(size/1024/1024).toFixed(1);var message=`File ${filename}size is ${sizeString}MB and the
upload limit is ${sizeLimit}MB.Please compress or reformat the file,because otherwise upload and validation errors are very likely.`;alertify.confirm("File size too large!",message,function(event){alertify.error("File upload cleared");},function(){alertify.error("File upload cleared");});}}
...
timers[uniqueId]=setTimeout(callback,ms);};})();function resizeAll(){$('input[type=text], input[type=number].sizeme, input[type=email], input[type=tel], input[type=date], input[type=time], input[type=url]').each(function(){if($(this).is(':visible')&&!['TD','TH','LABEL'].includes($(this).parent()[0].tagName)){$(this).width($(this).parent().width()-10);}});$('textarea').each(function(){$(this).width($(this).parent().width()-10);});$('.chosen-container').each(function(){if($(this).parent().is("td")||$(this).parent().is("th")){}else{$(this).width($(this).parent().width()-10);}});}
