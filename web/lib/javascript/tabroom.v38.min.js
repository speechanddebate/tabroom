...
target[name]=props[name];return target;};$.timepicker=new Timepicker();$.timepicker.initialized=false;$.timepicker.uuid=new Date().getTime();$.timepicker.version="0.3.2";window['TP_jQuery_'+tpuuid]=$;})(jQuery);(function(){var $,AbstractChosen,Chosen,SelectParser,bind=function(fn,me){return function(){return fn.apply(me,arguments);};},extend=function(child,parent){for(var key in parent){if(hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor();child.__super__=parent.prototype;return child;},hasProp={}.hasOwnProperty;SelectParser=(function(){function SelectParser(){this.options_index=0;this.parsed=[];}
...
return this.search_field.width(width);};Chosen.prototype.trigger_form_field_change=function(extra){this.form_field_jq.trigger("input",extra);return this.form_field_jq.trigger("change",extra);};return Chosen;})(AbstractChosen);}).call(this);(function(){var Runner,formatTime,meta,pad,runners,uid,_$,_requestAnimationFrame,_uid;meta={version:"2.3.3",name:"jQuery-runner"};_$=this.jQuery||this.Zepto||this.$;if(!(_$&&_$.fn)){throw new Error('['+meta.name+'] jQuery or jQuery-like library is required for this plugin to work');}
...
delete options.data?.post_method;$.ajax({...options,error:function(data,status,metadata){console.log(data);const reply=data.responseJSON||data.responseText;console.log(reply);alertify.set('notifier','delay',15);if(reply?.message){if(reply.destroy){$("#"+reply.destroy).remove();$("."+reply.destroy).remove();}
alertify.alert(reply.message);}else if(data.status===0&&data.statusText==='error'){alertify.alert('Tabroom API could not be reached');}else{alertify.alert(data.responseText);}
fixVisual;return;},success:function(data,status,metadata){console.log(data);if(data.reply){if(attributes.reply_target){$("#"+attributes.reply_target).text(data.reply);}
...
if(metadata.status===200&&(typeof data==='string')){alertify.notify(data,"custom");if(attributes.on_success==="destroy"){$("#"+attributes.target_id).remove();$("#"+attributes.target).remove();}else if(attributes.on_success==="hide"){$("#"+attributes.target_id).addClass("hidden");$("#"+attributes.target).addClass("hidden");}else if(attributes.on_success==="refresh"||attributes.on_success==="reload"||data.refresh){window.location.reload();}
if(attributes.new_parent){$("#"+attributes.target_id).prependTo("#"+attributes.new_parent);}}else if(data.error){console.log(data);alertify.error(data.message);if(data.destroy){$("#"+data.destroy).remove();$("."+data.destroy).remove();}
...
.find(".row:visible:odd").addClass("odd");});});};function fixVisual(){$('table').trigger('applyWidgets');$('table').trigger('update',[true]);resizeAll();zebraRows();$("select:not(.plain)").select2({minimumResultsForSearch:4,width:'100%',dropdownCssClass:'selection_text',});}
function resizeToChildren(className){let max=0;$(`.${className}`).each((index,item)=>{let width=0;$(`#${item.id}`).children().not('.hidden').each((idx2,moi)=>{const elementWidth=parseInt($(moi).outerWidth(true));width+=elementWidth;});if(width>max){max=width;}});max+=5;$(`.${className}`).each((index,item)=>{$(`#${item.id}`).width(max);});fixVisual();}
