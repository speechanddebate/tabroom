...
return old.apply(this,arguments);};})($.fn.attrs);function postSwitch(checkObject,url,callback,confirmMessage){if(confirmMessage!=undefined&&confirmMessage!=""){alertify.confirm("Please confirm",confirmMessage,function(e){if(e){}else{return;}},function(no){return;});}
...
const options={mode:'cors',credentials:'include',headers:{'Content-Type':'application/json','Accept':'application/json',},redirect:'follow',referrerPolicy:'no-referrer',};if(attributes.post_method==="get"){options.method='GET';}else if(attributes.post_method==="delete"){options.method='DELETE';options.body=JSON.stringify(attributes);}else if(attributes.post_method==="put"){options.method='PUT';options.body=JSON.stringify(attributes);}else{options.method='POST';options.body=JSON.stringify(attributes);}
console.log(options.method);$.ajax({type:options.method,data:attributes,crossDomain:true,xhrFields:{withCredentials:true},url}).then(function(data){if(data.reply){if(attributes.reply_target){$("#"+attributes.reply_target).text(data.reply);}
