...
if(attributes.new_parent){$("#"+attributes.target_id).prependTo("#"+attributes.new_parent);}}else if(data.error){alertify.error(data.message);if(data.destroy){$("#"+data.destroy).remove();$("."+data.destroy).remove();}
...
if(data.replace){console.log(data.replace);data.replace.forEach(function(item){if(item.destroy){$("#"+item.id).remove();}else if(item.content){$("#"+item.id).html(item.content);}});}
...
function resizeToChildren(className){let max=0;$(`.${className}`).each((index,item)=>{let width=0;$(`#${item.id}`).children('a').children().children(':visible').each((idx2,moi)=>{const elementWidth=parseInt($(moi).outerWidth(true));width+=elementWidth;});if(width>max){max=width;}});max+=5;$(`.${className}`).each((index,item)=>{$(`#${item.id}`).width(max);});}
