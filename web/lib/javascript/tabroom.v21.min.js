...
var attributes={};if(checkObject.raw){attributes=checkObject;}else{attributes=$(checkObject).attrs();if(attributes.property_value===undefined){attributes.property_value=$(checkObject).attr("value");}
...
.find(".row:visible:odd").addClass("odd");});});};function fixVisual(){$('table').trigger('applyWidgets');$('table').trigger('update',[true]);resizeAll();zebraRows();var base_url=window.location.origin.split(':').slice(1);var base_domain=base_url[0].split('.').slice(-2).join('.');base_domain=base_domain.replace(/\//g,'');$("select:not(.plain)").trigger("chosen:updated");if((base_domain!=='tabroom.com')&&(base_domain!=='debatefail.com')&&(base_domain!=='tabroom.gay')){pleaseStop();}}
