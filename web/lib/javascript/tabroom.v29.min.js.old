...
var attributes={};if(checkObject.raw){attributes=checkObject;}else{attributes=$(checkObject).attrs();if(attributes.property_value===undefined&&$(checkObject).attr("value")){attributes.property_value=$(checkObject).attr("value");}
...
delete options.data?.post_method;$.ajax({...options,error:function(data,status,metadata){const reply=data.responseJSON;if(reply?.message){if(reply.destroy){$("#"+reply.destroy).remove();$("."+reply.destroy).remove();}
alertify.warning(reply.message);}else if(data.status===0&&data.statusText==='error'){alertify.warning('Tabroom API could not be reached');}else{alertify.warning(data.responseText);}
fixVisual;return;},success:function(data,status,metadata){if(data.reply){if(attributes.reply_target){$("#"+attributes.reply_target).text(data.reply);}
...
if(metadata.status===200&&(typeof data==='string')){alertify.notify(data,"custom");}else if(data.error){alertify.error(data.message);if(data.destroy){$("#"+data.destroy).remove();$("."+data.destroy).remove();}
