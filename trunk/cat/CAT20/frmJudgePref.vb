...
Public Class frmJudgePref
    ...
    Sub MadeChange() Handles DataGridView1.CellEndEdit
        ...
        'sort before calcuating
        DataGridView1.Sort(DataGridView1.Columns("Rating"), System.ComponentModel.ListSortDirection.Ascending)
    ...
    Private Sub butBasicInfo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butBasicInfo.Click
        ...
        strInfo &= "WRITE DIVISION CONFLICTS TO PREFS: This will enter a rating of strike for all lower-division (i.e., novice) judges against all teams in higher divisions (i.e., open).   This is necessary for the STA judge placement system to work, but will erase preference information." & Chr(10) & Chr(10)
    ...
    Private Sub butOnlineUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butOnlineUpdate.Click
        If chkJudgeOnly.Checked = True And cboJudge.SelectedIndex = -1 Then
            Dim q As Integer = MsgBox("This will ONLY update the judge pref info for the judge selected in the school conflict box.  Please check that you have the desired judge selected.  Continue?", MsgBoxStyle.YesNo)
        ...
        If chkUpdateDisplayedOnly.Checked = True Then
            strSearch = "Team=" & cboTeam.SelectedValue.ToString
        Else
            strSearch = "Team>-1"
        End If
        If chkJudgeOnly.Checked = True Then strSearch &= " and judge=" & cboJudge.SelectedValue.ToString
        fdCATPref = ds.Tables("JudgePref").Select(strSearch)
        ...
        For x = 0 To fdCATPref.Length - 1
            ...
            If dr.Length = 0 Then
                ...
                If Not drOvTm Is Nothing Then
                    drErr.Item("Team") = drOvTm.Item("Code")
                Else
                    drErr.Item("Team") = "(Dropped team)"
                End If
        ...
        MsgBox("Done.  If you are using ordinals, click the recalculate ordinals button.  If you are using tiers (1-6, 1-9, etc.) go to the manual changes screen and click the ORD->RAT button.")
    End Sub
    ...
    Private Sub butDivConflicts_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butDivConflicts.Click
        Dim dr, fdTm As DataRow()
        For x = 0 To ds.Tables("Judge").Rows.Count - 1
            For y = 0 To ds.Tables("Event").Rows.Count - 1
                If ds.Tables("Judge").Rows(x).Item("Event" & ds.Tables("Event").Rows(y).Item("ID")) = False Then
                    fdTm = ds.Tables("Entry").Select("Event=" & ds.Tables("Event").Rows(y).Item("ID"))
                    For z = 0 To fdTm.Length - 1
                        dr = ds.Tables("JudgePref").Select("Team=" & fdTm(z).Item("ID") & " and Judge=" & ds.Tables("Judge").Rows(x).Item("ID"))
