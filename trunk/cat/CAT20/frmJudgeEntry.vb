Public Class frmJudgeEntry
    ...
    End Sub
    Sub NewJudgeCheck()
        'for this to work with tabroom, new judges need a really low ID number and the user then has to link the account up to the judge
        'New judges will have no downloadrecord, so that's how you find 'em
        'get the low number
        For x = 0 To ds.Tables("Judge").Rows.Count - 1
            If ds.Tables("Judge").Rows(x).Item("DownLoadRecord") Is System.DBNull.Value Then ds.Tables("Judge").Rows(x).Item("DownLoadRecord") = 0
            If ds.Tables("Judge").Rows(x).Item("DownLoadRecord") = 0 Then
                MsgBox("You have added new judges; if you wish them to be able to access ballots online you will need to link them to a judge account.  Log in to tabroom.com, click on the tournament, and from the Director's Page click Entries->Judges->Link Judges to Online Accounts.  The final link will be in the right-hand column.")
                ds.EnforceConstraints = False
                ds.Tables("Judge").Rows(x).Item("ID") = GetLowBlank()
                ds.Tables("Judge").Rows(x).Item("DownLoadRecord") = ds.Tables("Judge").Rows(x).Item("ID")
                ds.EnforceConstraints = True
            End If
        Next x
    End Sub
    Function GetLowBlank()
        GetLowBlank = -99
        Dim dr As DataRow
        For x = 1 To ds.Tables("Judge").Rows.Count + 100
            dr = ds.Tables("Judge").Rows.Find(x)
            If dr Is Nothing Then GetLowBlank = x : Exit Function
        Next x
    End Function
