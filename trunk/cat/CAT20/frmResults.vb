...
Public Class frmResults
    ...
    Private Sub frmResults_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'bind round CBO
        ds.Tables("Round").DefaultView.RowFilter = "Rd_Name<=9"
        cboround.DataSource = ds.Tables("Round").DefaultView
    ...
    End Sub
    Private Sub frmResults_Unload(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Disposed
        ds.Tables("Round").DefaultView.RowFilter = ""
    End Sub
        ...
        For x = 0 To dvRound.Count - 1
            ...
            For y = 0 To foundPanel.Length - 1
                ...
                For z = 0 To foundBallot.Length - 1
                    ...
                    If z < foundBallot.Length - 1 Then If foundBallot(z).Item("Judge") <> foundBallot(z + 1).Item("Judge") Then stw.WriteLine("    </BALLOT>")
                Next z
            Next y
    ...
    Private Sub butShowPrefByTeam_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butShowPrefByTeam.Click
        ...
        For x = 0 To DataGridView1.RowCount - 1
            ...
            For y = 0 To DataGridView1.Columns.Count - 1
                ...
                End If
            Next
            DataGridView1.Rows(x).Cells("Avg").Value = FormatNumber(Tot / ctr, 1)
        Next
    End Sub

    Private Sub butCheckers_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butCheckers.Click
        Dim dt As New DataTable
        Dim dummy As Integer = cboround.SelectedValue
        Dim strNameType As String = "FULLNAME"
        dt = MakePairingTable(ds, cboround.SelectedValue, "CODE")
        dt.Columns.Add(("Decision"), System.Type.GetType("System.String"))
        DataGridView1.DataSource = dt
        dt.DefaultView.Sort = "JudgeName1 ASC"
        Dim dr As DataRow : dr = ds.Tables("Round").Rows.Find(cboround.SelectedValue) : Dim nJudges As Integer = dr.Item("JudgesPerPanel")
        Dim nDebaters As Integer = getEventSetting(ds, dr.Item("Event"), "DebatersPerTeam")
        Dim nTeams As Integer = getEventSetting(ds, dr.Item("Event"), "TeamsPerRound")
        For team = 1 To nTeams
            For judge = 1 To nJudges
                For debater = 1 To nDebaters
                    For x = 0 To DataGridView1.RowCount - 1
                        DataGridView1.Rows(x).Cells("Team" & team & "spkr" & debater & "Name").Value = DataGridView1.Rows(x).Cells("Team" & team & "spkr" & debater & "Name").Value & " " & DataGridView1.Rows(x).Cells("Judge" & judge & "Team" & team & "spkr" & debater & "Speaker Points").Value
                        If DataGridView1.Columns.Contains("Judge" & judge & "Team" & team & "spkr" & debater & "Speaker Rank") Then
                            DataGridView1.Rows(x).Cells("Team" & team & "spkr" & debater & "Name").Value = DataGridView1.Rows(x).Cells("Team" & team & "spkr" & debater & "Name").Value & "/" & DataGridView1.Rows(x).Cells("Judge" & judge & "Team" & team & "spkr" & debater & "Speaker Rank").Value
                        End If
                        If Not DataGridView1.Rows(x).Cells("Judge1Team1Ballot").Value Is System.DBNull.Value Then
                            If DataGridView1.Rows(x).Cells("Judge1Team1Ballot").Value = 1 Then
                                DataGridView1.Rows(x).Cells("Decision").Value = "AFF -- " & DataGridView1.Rows(x).Cells("TeamName1").Value
                            Else
                                DataGridView1.Rows(x).Cells("Decision").Value = "NEG -- " & DataGridView1.Rows(x).Cells("TeamName2").Value
                            End If
                        End If
                    Next x
                Next debater
        ...
        For x = 0 To DataGridView1.ColumnCount - 1
            ...
            If InStr(DataGridView1.Columns(x).HeaderText.ToUpper, "JUDGENAME") > 0 Then DataGridView1.Columns(x).Visible = True
            If InStr(DataGridView1.Columns(x).HeaderText.ToUpper, "SPKR") > 0 And InStr(DataGridView1.Columns(x).HeaderText.ToUpper, "NAME") > 0 Then DataGridView1.Columns(x).Visible = True
            If InStr(DataGridView1.Columns(x).HeaderText.ToUpper, "DECISION") > 0 Then DataGridView1.Columns(x).Visible = True
            If DataGridView1.Columns(x).HeaderText.ToUpper = "TEAM1SPKR1NAME" Then DataGridView1.Columns(x).HeaderText = "Aff 1"
            If DataGridView1.Columns(x).HeaderText.ToUpper = "TEAM1SPKR2NAME" Then DataGridView1.Columns(x).HeaderText = "Aff 2"
            If DataGridView1.Columns(x).HeaderText.ToUpper = "TEAM2SPKR1NAME" Then DataGridView1.Columns(x).HeaderText = "Neg 1"
            If DataGridView1.Columns(x).HeaderText.ToUpper = "TEAM2SPKR2NAME" Then DataGridView1.Columns(x).HeaderText = "Neg 2"
            If DataGridView1.Columns(x).HeaderText.ToUpper = "JUDGENAME1" Then DataGridView1.Columns(x).HeaderText = "Judge"
        Next x
    End Sub

    Private Sub butWinsBySchool_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butWinsBySchool.Click
        Dim dtFinal As New DataTable
        dtFinal.Columns.Add(("School"), System.Type.GetType("System.String"))
        dtFinal.Columns.Add(("Wins"), System.Type.GetType("System.Int16"))
        Dim drFinal As DataRow
        Dim dt As DataTable : Dim dr As DataRow
        Dim rd2 As Integer = cboround.SelectedValue
        dt = MakeTBTable(ds, cboround.SelectedValue, "TEAM", "CODE", cboTieBreakSet.SelectedValue, rd2)
        For x = 0 To ds.Tables("School").Rows.Count - 1
            drFinal = dtFinal.NewRow : drFinal.Item("School") = ds.Tables("School").Rows(x).Item("SchoolName") : drFinal.Item("wins") = 0
            For y = 0 To dt.Rows.Count - 1
                dr = ds.Tables("Entry").Rows.Find(dt.Rows(y).Item("competitor"))
                If dr.Item("school") = ds.Tables("school").Rows(x).Item("id") Then
                    drFinal.Item("wins") += dt.Rows(y).Item("Wins")
                End If
            Next y
