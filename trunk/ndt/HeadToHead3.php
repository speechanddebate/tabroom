...
$sum_team=array(); $sum_win=array(); $sum_loss=array(); $sum_team_id=array();
...
//fill in opponent name and id (panel_entry_sync), decisions, judge
 $x=0; $opponum = array(); $oppname = array(); $judge=array(); $decision=array(); $opponent=0; 
 while ($x<count($panel_id))
  {
   $x++;
   getopponent($panel_id[$x], $panel_entry_orig[$x], $oppname[$x], $judge[$x], $decision[$x], $opponent);
   $opponum[$x]=$opponent;
   }
 array_multisort($oppname, $opponum, $panel_entry_sync, $panel_entry_orig, $panel_entry_waitlist, $panel_id, $judge, $decision);
   ...
   while ($row = mysql_fetch_array($entries, MYSQL_BOTH)) 
    {
     ...
     if ($displayOK==TRUE) 
      ...
      echo "</tr></thead><tbody>";
      printdebates($row['id'], $panel_id, $panel_entry_sync, $panel_entry_orig, $win, $loss, $mode, $panel_entry_waitlist, $opponum, $oppname, $decision, $judge);
      echo "</table>";
      if ($displayOK==TRUE) {echo " (".$win."-".$loss.")</br>";}
      $sum_team[$x]=$row['name']; $sum_win[$x]=$win; $sum_loss[$x]=$loss; $sum_team_id[$x]=$panel_entry_sync[$x];
      } 
      $x++; 
    }
?>
...
function printdebates($entry, $panel_id, $panel_entry_sync, $panel_entry_orig, &$win, &$loss, $mode, $panel_entry_waitlist, $opponum, $oppname, $decision, $judge)
{
 ...
 while ($x<count($panel_id))
  {
   if ($entry==$panel_entry_sync[$x] and nduplicates($panel_id[$x], $panel_id)>1)
    {
    $oppwaitlist=getoppwaitlist($panel_entry_orig, $panel_entry_waitlist, $opponum[$x]);
    $printok=FALSE;
    ...
    if ($printok==TRUE)
     ...
     echo "<td>".$oppname[$x]."</td>";
     echo "<td>".$judge[$x]."</td>";
     echo "<td>".$decision[$x]."</td>";
     if (iswin($decision[$x])=="W") {$win++;} else {$loss++;}
     echo "</tr>";
     }
    }
   $x++;
...
<h2>Summary Table<span style="font-size:24px"><a href="#Top"> Jump to top</a></span></h2>
	...
	<tr class="yellowrow">
		...
		<th class="smallish">HtH Win</th>
		<th class="smallish">HtH Loss</th>
		<th class="smallish">HtH Pct</th>
		<th class="smallish">All Win</th>
		<th class="smallish">All Loss</th>
		<th class="smallish">All Pct</th>
	</tr>
	</thead>
...
if (isset($sum_team[$i]))
 ...
 $win=0; $loss=0;
 getwins($sum_team_id[$i], $panel_entry_sync, $decision, $win, $loss, $panel_id);
 echo "<td>".$win."</td>";
 echo "<td>".$loss."</td>";
 echo "</tr>";
 }
...
<?php
function getwins($entry, $panel_entry_synch, $decision, &$win, &$loss, $panel_id)
{
 $i=1; $ctr=0;
 while ($i<count($panel_entry_synch))
  {
   if ($panel_entry_synch[$i]==$entry)
    {
     $ctr++;
     //echo $ctr." ".getround($panel_id[$i])." ".iswin($decision[$i])."<br>";
     if (iswin($decision[$i])=="W") {$win++;} else {$loss++;}
    }
   $i++;
  }
}

function getround($panel_id)
{
  $query="SELECT *, round.label as round_label, tourn.name as tourn_name from round, tourn, panel, event where event.id=round.event and tourn.id=event.tourn and round.id=panel.round and panel.id=".$panel_id;
  //echo $query."<br>";
  $ballots=mysql_query($query);
  while ($row = mysql_fetch_array($ballots, MYSQL_BOTH)) 
    {
     return $row['tourn_name']." ".$row['round_label'];
    }
}
